...
for db in $DATABASES; do
  ...
  if [ "$db" = "periscope" ]; then
    run_section "${db}: ledger rebuild cursors" "SELECT ledger_name, last_processed_projection_ms, toDateTime(last_processed_projection_ms / 1000) AS last_processed_at, updated_at_ms, toDateTime(updated_at_ms / 1000) AS updated_at FROM ${db}.ledger_rebuild_cursors_v2 FINAL ORDER BY ledger_name FORMAT PrettyCompact"
