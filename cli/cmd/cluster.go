...
invocation. Explicit flags always win over saved context defaults.`,
	...
	cluster.AddCommand(newClusterSecretsCmd())
...
type resolvedCluster struct {
	...
	sharedEnvErr          error
	preparedSharedEnvOnce sync.Once
	preparedSharedEnvErr  error
...
// PreparedSharedEnv is the single lifecycle-command contract for shared
// credentials. Production validates operator-owned SOPS input. Development
// persists only values generated by the CLI, so separate provision/apply/
// restart processes never derive different authority or encryption roots.
func (rc *resolvedCluster) PreparedSharedEnv() (map[string]string, error) {
	rc.preparedSharedEnvOnce.Do(func() {
		env, err := rc.SharedEnv()
		if err != nil {
			rc.preparedSharedEnvErr = err
			return
		}
		if !isDevProfile(rc.Manifest) {
			rc.preparedSharedEnvErr = credentials.ValidateShared(env)
			return
		}
		if !rc.SourcePersistsManifest || strings.TrimSpace(rc.ManifestPath) == "" {
			rc.preparedSharedEnvErr = fmt.Errorf("development secret generation requires a persistent local manifest source")
			return
		}
		cachePath := filepath.Join(filepath.Dir(rc.ManifestPath), devGeneratedSecretsRelativePath)
		cached, loadErr := loadDevGeneratedSecrets(cachePath)
		if loadErr != nil {
			rc.preparedSharedEnvErr = loadErr
			return
		}
		owned := make(map[string]string)
		for _, key := range credentials.Keys() {
			if strings.TrimSpace(env[key]) == "" {
				if value := strings.TrimSpace(cached[key]); value != "" {
					env[key] = value
					owned[key] = value
				}
			}
		}
		generated, genErr := credentials.GenerateIfMissing(env)
		if genErr != nil {
			rc.preparedSharedEnvErr = genErr
			return
		}
		...
		if len(owned) > 0 {
			rc.preparedSharedEnvErr = persistDevGeneratedSecrets(cachePath, owned)
		}
	})
	return rc.sharedEnv, rc.preparedSharedEnvErr
}

func loadDevGeneratedSecrets(path string) (map[string]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("inspect generated development secrets: %w", err)
	}
	if info.Mode().Perm()&0o077 != 0 {
		return nil, fmt.Errorf("generated development secrets %s must have mode 0600", path)
	}
	data, err := os.ReadFile(path)
	if err != nil {
		return nil, fmt.Errorf("read generated development secrets: %w", err)
	}
...
func persistDevGeneratedSecrets(path string, values map[string]string) error {
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
		return fmt.Errorf("create generated development secret directory: %w", err)
	}
	...
	content.WriteString("# Generated by the FrameWorks CLI for this development manifest.\n")
	...
	tmp, err := os.CreateTemp(filepath.Dir(path), ".dev-generated-secrets-*")
	if err != nil {
		return fmt.Errorf("create generated development secret staging file: %w", err)
	}
	tmpPath := tmp.Name()
	defer os.Remove(tmpPath) //nolint:errcheck // staging cleanup only
	if chmodErr := tmp.Chmod(0o600); chmodErr != nil {
		_ = tmp.Close()
		return chmodErr
	}
	if _, writeErr := io.WriteString(tmp, content.String()); writeErr != nil {
		_ = tmp.Close()
		return writeErr
	}
	...
	if renameErr := os.Rename(tmpPath, path); renameErr != nil {
		return fmt.Errorf("install generated development secrets: %w", renameErr)
	}
