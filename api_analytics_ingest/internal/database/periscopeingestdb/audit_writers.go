...
func PrepareAPIEvent(ctx context.Context, db BatchPreparer) (*Writer[APIEventRow], error) {
	return prepare(ctx, db, insertAPIEvent, func(row APIEventRow) []interface{} {
		return []interface{}{row.EventID, row.TenantID, row.EventType, row.Source, row.UserID, row.ResourceType, row.ResourceID, row.Details, row.Timestamp, row.ClusterID, row.SourceRegion, row.StreamOriginRegion, row.StreamOriginClusterID, row.SchemaVersion, row.ActorAuthType, row.ActorTokenHash}
	})
}
