...
func TestSchemaDiffBreakingRules(t *testing.T) {
	cases := []struct {
		...
		{"optional argument gained a default", "stream(id: ID!, verbose: Boolean)", "stream(id: ID!, verbose: Boolean = false)", "argument Query.stream(verbose:) gained the default false; a request that omits it now reaches the resolver with that value instead of none"},
		{"optional input field gained a default", "record: Boolean mode", "record: Boolean = true mode", "input field CreateStreamInput.record gained the default true; a request that omits it now reaches the resolver with that value instead of none"},
