...
<Player contentId="pk_..." contentType="live" options={{ locale: "de" }} />
```

```ts
// Vanilla — built-in locale
...
<Aside type="note" title="Custom player translations">
  The player translation engine supports custom translation overrides internally, but the current
  React, Svelte, and vanilla public facades only wire built-in locale selection.
</Aside>
