...
# Vanilla JS / Other frameworks (headless, no UI)
...
For a fully-skinned player with no bundler required, load the IIFE bundle via a free npm CDN:
...
Shadow DOM encapsulates all player CSS — host page styles won't break controls. Theme via CSS custom properties:
