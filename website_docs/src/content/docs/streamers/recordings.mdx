...
Recording availability and retention have two layers:

- Runtime DVR starts default to a 30-day expiration when no `expiresAt` is supplied.
- The billing catalog seeds Free without DVR recording and paid plan policy with longer windows
  (Supporter 90 days, Developer 180 days, Production 365 days, Enterprise custom).

Current beta enforcement of billing-tier recording entitlements is not uniform across every DVR
entry point. Treat the billing table as plan policy; runtime behavior is governed by the DVR request
and cluster retention configuration until commercial enforcement is fully enabled. Foghorn's hourly
retention job marks expired artifacts as deleted, then cleanup jobs remove files and stale records.
