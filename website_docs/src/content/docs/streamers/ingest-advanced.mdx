...
```tsx
// React
<StreamCrafter whipUrl="..." useWebCodecs={false} />;
...
const client = new IngestClient({ gatewayUrl: "%GATEWAY_URL%/graphql" });
const { whipUrl } = await client.resolveEndpoint("my-stream-key");
...
// Custom MediaStream
await studio.controller.addSource(customStream, {
