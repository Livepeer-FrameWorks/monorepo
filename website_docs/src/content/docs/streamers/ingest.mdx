...
# or Web Components (Vue, Angular, CDN, plain HTML)
...
For a fully-skinned streaming UI with no bundler, load the IIFE bundle via a free npm CDN:
...
Or install via npm and use the ESM entry:
...
Shadow DOM encapsulates all StreamCrafter CSS — host page styles won't break controls. The component includes the same full UI as the React/Svelte versions: video preview, camera/screen controls, quality selection, and connection status.
