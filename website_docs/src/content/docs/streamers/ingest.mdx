...
```tsx
<StreamCrafter
  whipUrl="%INGEST_URL%/webrtc/your-stream-key"
  initialProfile="broadcast"
/>
```
...
- Gateway mode is key‑based and resolves the best ingest endpoint for your stream.
</Aside>
...
Quality profile changes apply new constraints and take effect after a short adaptation (typically by the next keyframe).
</Aside>
...
Background‑safe encoding requires WebCodecs **and** RTCRtpScriptTransform support (Chromium‑family). Without that path, browser encoders may pause when the tab loses focus.
</Aside>
