...
| ↳ Backend SDKs | Expanding | Dashboard, Docs | SDKs for TypeScript (Node 22+ and browsers), Go, and Python, generated from the public GraphQL schema and released under one version. Each has a generated typed operation for every public root query, mutation, and subscription and every argument-taking field below Query, with a default selection; TypeScript adds typed custom selections over the whole public schema (the select entry), Go and Python take custom documents through the client. Around them are retries, pagination, typed errors, a server version check, a multipart VOD upload helper, playback JWT signing, and Standard Webhooks verification with typed event parsing. |
