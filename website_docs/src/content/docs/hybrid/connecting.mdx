...
1. Creates a private cluster (VPC) if you don't have one
2. Generates an enrollment token
3. Pre-registers the edge node (assigns domain, ID, and internal gRPC trust bootstrap)
...
- **Docker container status** for MistServer, Helmsman, and Caddy
...
Streamers can push directly to your node instead of the default ingest. Direct ingest requires MistServer ports to be exposed on the host. The default Docker edge template publishes only 80/443, so expose the non-HTTP ports in your deployment before using direct RTMP/SRT/RTSP ingest.
