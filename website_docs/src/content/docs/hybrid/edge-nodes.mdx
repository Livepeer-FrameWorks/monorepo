...
When it finishes, `edge deploy` prints a five-line result block (vpc / enrollment / stack / https / node / domain) and suggests `edge status` and `edge doctor` as next steps.
