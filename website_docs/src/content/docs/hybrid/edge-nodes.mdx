...
When it finishes, `edge deploy` prints a result block (`vpc`, `enrollment`, `stack`, `https`, `node`, `domain`) and suggests `edge status` and `edge doctor` as next steps. The `vpc` label is the current CLI output name for the private edge cluster step.
