...
When using `--enrollment-token`, the CLI calls Foghorn's `PreRegisterEdge` RPC to get an assigned domain, node ID, cluster slug, and the internal CA bundle used for gRPC trust bootstrap. You still need `--foghorn-addr` (or have it set in your CLI context) to reach Foghorn. If you already have a domain and don't need auto-assignment, use `--domain` instead:
