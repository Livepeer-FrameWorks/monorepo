...
**What to backup (container mode):** the whole project directory and the
Helmsman state. Together they carry the node's identity and secrets:

- `.edge.env` - Configuration (node ID, domain, control-plane address)
...
- `docker-compose.edge.yml` (or `docker-compose.yml` on Ansible-provisioned
  nodes)
- **Required:** Helmsman state mounted at `/data/state` — the `edge_state`
  named volume on local `edge init` renders, or the `state/` directory next
  to the compose file on Ansible-provisioned nodes. It holds the node's
  Ed25519 identity key and enrollment receipt, plus the persisted config
  seed, trigger WAL, and control outbox.

If the state is lost, Helmsman generates a new identity key and the
control plane rejects the node because the key no longer matches its registration. Recover
by re-enrolling with a fresh `edge_node` token and
`HELMSMAN_ROTATE_NODE_IDENTITY=true`, as described in
[Recover a legacy keyless node](/hybrid/connecting#recover-a-legacy-keyless-node).
...
3. Restore the saved project directory (env files, `pki/`, `telemetry/`,
   compose file) and the Helmsman state (`edge_state` volume or `state/`
   directory) — do **not** re-run `edge init`, which would mint a new node
