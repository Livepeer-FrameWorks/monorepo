...
DVR chapters turn one continuous recording into smaller playback windows. Each chapter is finalized as a canonical `.mkv` VOD artifact with a matching `.dtsh` seek index and runs through the tenant's standard VOD processing pipeline (Thumbs, sprite, optional Livepeer transcodes) just like a user-initiated VOD upload.
...
Chapter mode is a property of the source **Stream**, snapshotted onto each recording at StartDVR. Changes apply to the next recording, not in-flight ones.
...
| Mode             | Best for                                                     |
| ---------------- | ------------------------------------------------------------ |
| `WINDOW_SIZED`   | Default replay buckets sized to the stream's DVR window.     |
| `FIXED_INTERVAL` | Regular buckets such as hourly, 6-hour, or 24-hour chapters. |
| `NONE`           | Chapters disabled for this stream.                           |
