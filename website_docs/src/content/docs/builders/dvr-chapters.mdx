...
DVR chapters turn one continuous recording into smaller playback windows. Each chapter is finalized as a canonical `.mkv` VOD artifact with a matching `.dtsh` seek index and fresh poster/sprite thumbnails for the chapter timeline. Chapters use the standard VOD playback path, but their processing pass is thumbs-only because the source recording already carries the live A/V renditions.
