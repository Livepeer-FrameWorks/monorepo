...
Public allowlist (no auth required): `serverInfo`, `resolveViewerEndpoint`, `resolveIngestEndpoint`, `networkStatus`, and the orchestrator queries (queries only).
...
| Status | Meaning                                                                                                            |
| ------ | ------------------------------------------------------------------------------------------------------------------ |
| 401    | The token is missing or invalid (`{"error": "authentication failed"}`).                                            |
| 402    | The operation needs credit, a payment method, or an x402 payment; `code` says which (`INSUFFICIENT_BALANCE`, ...). |
