...
Pull-input streams are not created by GraphQL today. Operators provision them through
`commodore.pull_streams` during bootstrap; see [Pull-input Streams](/builders/pull-streams).
...
FrameWorks does not publish a first-party GraphQL SDK yet. Use any GraphQL client in your language,
or generate typed bindings from the schema.
