...
Record your live streams, replay long-running DVR archives, extract clips, and upload pre-recorded content for on-demand playback.
...
Retrieve a single chapter manifest URL — embed it in a `<video>` element or any HLS player:
...
`manifestUrl` is a presigned GET URL for the chapter playback manifest. `isCurrent: true` means the chapter is still being recorded into and the manifest is `EVENT`-shaped (no `#EXT-X-ENDLIST`). For `FIXED_INTERVAL` mode pass `intervalSeconds`; for civil-day chapters resolve the local-day to a UTC range in your client and submit as `EXPLICIT_RANGE`.
...
<Aside type="note" title="VOD storage lifecycle">
  VOD assets also count toward stored bytes while retained. The current upload registry defaults VOD
  retention to 90 days unless your plan, operator policy, or a future API override changes it.
</Aside>
