...
For stream-scoped media views, chapter artifacts also surface through `storageArtifactsConnection` when you request `kinds: [VOD, CHAPTER]` and pass a `streamId`. Omit `streamId` to get the normal tenant VOD library, which only returns library-visible user uploads.
...
Follow clips in real time with the clip events of `tenantEvents`:

```graphql
subscription {
  tenantEvents(
    types: ["clip.requested", "clip.ready", "clip.failed"]
    streamId: "stream-global-id"
  ) {
    type
    time
...
The same subscription with `types: ["recording.ready", "recording.failed"]` reports finalized and
failed DVR recordings. See [Events](/builders/events) for every event type.
...
query {
  storageArtifactsConnection(
    input: { kinds: [CLIP], streamId: "stream-global-id", first: 20, offset: 0 }
  ) {
    nodes {
      id
      title
...
query {
  storageArtifactsConnection(input: { kinds: [VOD, CHAPTER], first: 20, offset: 0 }) {
    nodes {
      id
      playbackId
      title
