...
<Aside type="note" title="No default gateway">
  ...
  Modes](/builders/playback-advanced/#source-resolution-modes) for when to use each option.
</Aside>

## Where to Resolve From

Call `resolveViewerEndpoint` from the viewer's browser or playback device when you want geographic
routing to reflect that viewer. The FrameWorks player does this for you when you provide
`gatewayUrl`: the browser calls Gateway GraphQL, Gateway forwards the request IP to Foghorn, and
Foghorn uses that IP for GeoIP and edge scoring.
...
For custom players that do not use the FrameWorks player, use the playback DNS name directly:

```bash
https://foghorn.{tenant}.cdn.{base}/play/{playback-id}/hls/index.m3u8
```

If a tenant alias is not available, use the global root playback domain, then the cluster-concrete
Foghorn domain shown in the dashboard. HTTP playback URLs can go through `/play` because Foghorn can
return JSON or issue a 307 redirect from the viewer's request.
...
```json
{
  "primary": {
    ...
    "base_url": "%EDGE_URL%",
    "outputs": {
      "HLS": {"url": "%PLAY_URL%/play/stream-id/hls/index.m3u8"},
      "WHEP": {"url": "%PLAY_URL%/play/stream-id/webrtc"}
    }
  },
  "fallbacks": [...]
}
...
Use the DVR `playbackId` with `contentType="dvr"` for the active live DVR window. For historical
replay beyond that window, request a chapter manifest from the recording archive and give that HLS
URL to your player. Chapters are bounded playlist views over the same DVR segment archive, so a
90-day recording is navigated one chapter or explicit range at a time instead of as one giant
playlist.

See [Recordings, Clips & VOD](/builders/recordings#archive-chapters) for the 24/7 DVR model.
...
`gatewayUrl` resolves to the correct Foghorn for that viewer's region.
