...
The player resolves through the official FrameWorks Gateway from the viewer's browser, so Foghorn
routes from that viewer's IP. Set `gatewayUrl` only for a provider-managed private Gateway or local
...
All sub-components read state from the nearest `<Player>` parent via `usePlayerContextOptional()`. Outside a `<Player>` they still render, but only reflect and act on the props you pass them (for example `isPlaying` and `onToggle` on `PlayButton`).
...
| `options.playbackMode`   | `string`                                             | Protocol selection mode (see [Playback Modes](#playback-modes)); change at runtime with `setDevModeOptions({ playbackMode })` |
...
With every output available, each mode typically selects:
...
```tsx
<Player contentId="a1b2c3d4e5f6g7h8" contentType="live" options={{ playbackMode: "low-latency" }} />
```
