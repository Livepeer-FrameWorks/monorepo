...
Resolve through Gateway from the viewer's browser or playback device. If your backend calls Gateway,
Foghorn routes using the backend or proxy IP, not the final viewer's location.
