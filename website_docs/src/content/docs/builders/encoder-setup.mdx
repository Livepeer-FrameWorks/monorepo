...
- **Self-hosted operators must provide transcoding capacity.** Configure a Livepeer Gateway if you
  want the default video ABR ladder on your own cluster.

<Aside type="note" title="When ABR Is Available">
  Video ABR renditions require the stream's process configuration and a Livepeer Gateway reachable
  by the cluster handling the stream. Managed FrameWorks clusters include this path; self-hosted
  clusters need to configure it.
</Aside>
