...
The element resolves through the official FrameWorks Gateway from the viewer's browser, so Foghorn
routes from that viewer's IP. Set `gateway-url` only for a provider-managed private Gateway or local
...
```html
<fw-player content-type="dvr" content-id="a1b2c3d4e5f6g7h8"></fw-player>
<fw-player content-type="clip" content-id="a1b2c3d4e5f6g7h8"></fw-player>
```
...
```html
<fw-player id="player" content-type="live" content-id="a1b2c3d4e5f6g7h8" autoplay muted></fw-player>
...
```html
<fw-player id="slot-player" content-id="a1b2c3d4e5f6g7h8" content-type="live">
  <div slot="controls">
...
```html
<fw-player id="my-player" content-id="a1b2c3d4e5f6g7h8" content-type="live"></fw-player>
...
| `content-type`             | `string`              | —           | `live`, `dvr`, `clip`, or `vod`; auto-detected from Gateway or Mist metadata when omitted                                                                                        |
...
| `playback-mode`            | `string`              | `"auto"`    | Protocol selection mode (see [Playback Modes](/builders/playback-advanced/#playback-modes)); changing only this attribute after the player has started does not re-run selection |
| `dev-mode`                 | `boolean`             | `false`     | Show dev panel                                                                                                                                                                   |
| `debug`                    | `boolean`             | `false`     | Debug logging                                                                                                                                                                    |

## Theming

Use a preset with the `theme` attribute:

```html
<fw-player theme="tokyo-night" ...></fw-player>
```

Or set individual tokens with the `themeOverrides` property:

```js
document.querySelector("fw-player").themeOverrides = {
  accent: "200 80% 60%",
  surface: "230 15% 12%",
  radius: "8px",
};
...
Setting `--fw-*` custom properties on the `fw-player` element from page CSS has no effect: the
player redeclares every token on its surface element inside the shadow root.
...
const contentId = ref("a1b2c3d4e5f6g7h8");
</script>
```
