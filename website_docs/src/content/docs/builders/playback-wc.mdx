...
The `gateway-url` request runs in the viewer's browser, so Foghorn routes from that viewer's IP. For
backend-generated playback URLs or custom players that do not use this element, use the dashboard
playback DNS URL directly.
