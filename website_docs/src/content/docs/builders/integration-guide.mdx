...
<Aside type="note" title="Studio Endpoint Resolution">
  StreamCrafter components also accept `gatewayUrl` + `streamKey` and call the public
  `resolveIngestEndpoint` GraphQL field. Use that mode only in environments where the ingest
  resolver is enabled; otherwise pass the dashboard WHIP URL as `whipUrl`.
</Aside>
...
```tsx
<Player contentId="pk_..." contentType="live" options={{ gatewayUrl: "...", theme: "tokyo-night" }}>
  <PlayButton />
  <SettingsMenu />
...
<html>
  <body>
    ...
    <fw-player content-id="pk_..." content-type="live" gateway-url="..." autoplay muted></fw-player>
