...
If you are still deciding what to create first, start with [Streams](/builders/streams) for the
basic object model: stream keys, playback IDs, push ingest, and pull-input streams.
...
These packages cover the browser side. To create streams, upload files, sign viewer tokens, or
verify webhooks from your backend, use the [SDKs](/builders/sdks): `@livepeer-frameworks/api`
(TypeScript), `github.com/Livepeer-FrameWorks/sdk-go` (Go), or `livepeer-frameworks` (Python).
...
```tsx
// Player
<Player contentId="a1b2c3d4e5f6g7h8" contentType="live" />
...
<Aside type="note" title="Studio endpoint resolution">
  StreamCrafter accepts `gatewayUrl` + `streamKey` and resolves the WHIP endpoint via the public
  `resolveIngestEndpoint` GraphQL field. Passing the dashboard WHIP URL directly as `whipUrl` also
  works and takes priority when both are set.
</Aside>
...
<fw-player content-id="a1b2c3d4e5f6g7h8"><div slot="controls"></div></fw-player>
...
| **Protocols**    | HLS, LL-HLS, DASH, WebRTC (WHEP), WebSocket | WebRTC (WHIP)                                 |
...
```mermaid
flowchart TB
    PC[player-core<br/>headless playback] --> PR[player-react]
    PC --> PS[player-svelte]
    PC --> PW[player-wc]

    SC[streamcrafter-core<br/>headless ingest] --> SR[streamcrafter-react]
    SC --> SS[streamcrafter-svelte]
    SC --> SW[streamcrafter-wc]
```
...
```tsx
<Player contentId="a1b2c3d4e5f6g7h8" contentType="live" options={{ theme: "tokyo-night" }}>
  <PlayButton />
  <SettingsMenu />
...
<html>
  <body>
    ...
    <fw-player content-id="a1b2c3d4e5f6g7h8" content-type="live" autoplay muted></fw-player>
