...
By default, resolution runs through the official FrameWorks Gateway from the viewer's browser or
playback device. Set `gatewayUrl` only for a provider-managed private Gateway or local Gateway
preview. If you are generating URLs on a backend or building a player that will not call Gateway
from the viewer, use the dashboard playback DNS URL directly.

## Queries (Read State)

| Property       | Type                                          | Description                                             |
| -------------- | --------------------------------------------- | ------------------------------------------------------- |
| `playerState`  | `PlayerState`                                 | Current lifecycle state (`state` is a deprecated alias) |
...
| `selectQuality(id)`    | `(id) => void`            | Lock to a quality by its `getQualities()` id (engine level/track index), or `"auto"` |
...
if (player.capabilities.qualitySelection) player.selectQuality(player.getQualities()[1].id);
```
...
| `contentType`    | `string`                            | —           | Content type; auto-detected from Gateway or Mist metadata when omitted                      |
...
| `playbackMode`   | `string`                            | `"auto"`    | Protocol selection mode (see [Playback Modes](/builders/playback-advanced/#playback-modes)) |
| `skin`           | `string \| SkinDefinition \| false` | `"default"` | Skin for blueprint rendering (see [Advanced](/builders/playback-advanced/))                 |
