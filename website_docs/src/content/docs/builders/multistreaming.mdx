...
Playback policy, DVR, and analytics stay on the single ingest; only distribution fans out.
...
| Platform    | Dashboard preset ingest URL                                            | Key source                  |
| ----------- | ---------------------------------------------------------------------- | --------------------------- |
| Twitch      | `rtmp://live.twitch.tv/app/`                                           | Creator Dashboard           |
| YouTube     | `rtmp://a.rtmp.youtube.com/live2/`                                     | Live Control Room           |
| Facebook    | `rtmps://live-api-s.facebook.com:443/rtmp/`                            | Live Producer               |
| Kick        | `rtmps://fa723fc1b171.global-contribute.live-video.net/app/`           | Creator Dashboard           |
| X (Twitter) | `rtmps://prod-ec-us-west-2.compose.broadcast.live-video.net:443/rtmp/` | Media Studio or Producer    |
| Custom      | None; supply a complete `rtmp://`, `rtmps://`, or `srt://` URI         | The destination's dashboard |

In the dashboard, a platform preset fixes the ingest URL shown above and asks only for the stream
key; FrameWorks stores the preset URL followed by the key. If the destination gives you a different
server URL (another region or ingest server), choose **Custom** and enter the complete URI.

Through the API, you always supply the complete target URI. The `platform` value there is only a
label for the destination and does not change the URI.
