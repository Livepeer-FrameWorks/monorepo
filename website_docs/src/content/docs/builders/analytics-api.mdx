...
<Aside type="note" title="Player startup QoE is shipping incrementally">
  Stream health, routing, geographic, usage, and client QoE summaries are available today. Player
  **startup** QoE from the FrameWorks npm player — time-to-first-frame waterfall and
  manifest/segment Resource Timing — ships as an opt-in beacon, surfaced via
  `analytics.health.playerBootSummary`. Still maturing: rebuffering and rendition-change capture,
  and QoE from non-npm / third-party players. See [View-Level QoE
