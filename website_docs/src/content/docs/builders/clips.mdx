...
Subscribe to `tenantEvents` with the clip event types for real-time request, readiness, and failure
events on your clips. Pass `streamId` to receive only clips of one stream:
...
`artifactId` is the clip's `hash`. See [Events](/builders/events) for every event type and payload
field.

## Listing clips

Clips are listed through the unified `storageArtifactsConnection` with `kinds: [CLIP]`. Pass `streamId` to scope to a single source stream, or omit it to list all clips for the tenant. Pagination is offset-based (`first`/`offset`).

```graphql
query MyClips {
  storageArtifactsConnection(input: { kinds: [CLIP], first: 20, offset: 0 }) {
    nodes {
      id
      key
      hash
