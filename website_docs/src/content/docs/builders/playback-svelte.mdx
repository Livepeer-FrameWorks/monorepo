...
| `options.playbackMode`   | `string`                                             | Protocol selection mode (see [Playback Modes](/builders/playback-advanced/#playback-modes)); change at runtime with the store's `setDevModeOptions({ playbackMode })` |
