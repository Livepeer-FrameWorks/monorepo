...
1. **RTMP/E-RTMP URL:** Verify it is exactly `%RTMP_URL%`.
...
The stream page updates from the `liveStreamEvents` subscription and falls back to polling every 60 seconds. If the subscription connection dropped, refresh the page.
...
The token is invalid, expired, or revoked, or the request carried a payment header without a credential. A request with no credential at all on a non-public operation gets HTTP 402 with an x402 challenge instead.

**For API tokens (programmatic access):**

1. Create a new token from **Developer → Credentials** in the dashboard
...
A missing permission is not an HTTP 403: the response is HTTP 200 with a GraphQL error whose `extensions.code` is `FORBIDDEN` (see [API Reference — Error Handling](/builders/api-reference#error-handling)). You don't have access to this resource.
...
4. **GitHub** - [Open an issue](%GITHUB_URL%/issues) with your stream ID and timestamps
