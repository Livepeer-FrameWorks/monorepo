...
FrameWorks billing is in beta. Paid plans are rated from metered usage, prepaid balances are deducted as usage is reported, and postpaid invoices are drafted for the active billing period. API request rate limits still apply for abuse protection.
...
<Aside type="tip">
  **API requests are free.** Stream creation, analytics queries, configuration changes—all API
  ...
  when viewers watch, when recordings are stored, or when AI GPU workloads run.
</Aside>
...
- 10,000 delivered minutes/month
...
| Storage (avg GB)  | €0.035/GB    |
| GPU Hours         | €1.50/hr     |

GPU-hours apply to AI workloads (live transcription, V2V transformations, automated highlights).
Standard transcoding and multiview compositing are bundled into delivered minutes.
...
| ---------- | ----------------- | ---------------- | --------- |
| Supporter  | €0.00055/min      | €0.035/GB        | €1.50/hr  |
| Developer  | €0.00052/min      | €0.030/GB        | €1.50/hr  |
| Production | €0.00050/min      | €0.025/GB        | €1.50/hr  |
...
Monthly subscription with included delivered-minute and GPU allocations. Storage is rated by average GB stored during the period. Overages are drafted during the billing period and invoiced at period end. Requires verified billing details.
