...
<Aside type="tip">
  **API requests are free.** Stream creation, analytics queries, configuration changes—all API
  ...
  when viewers watch or when recordings are stored under the public beta catalog. Advanced
  processing (AI, V2V, compositing) is in pilot — see below.
</Aside>
...
Monthly subscription with included delivered-minute allocations. Storage is rated by average GB stored during the period. Overages are drafted during the billing period and invoiced at period end. Requires verified billing details.
