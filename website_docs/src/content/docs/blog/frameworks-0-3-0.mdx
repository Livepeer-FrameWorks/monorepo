...
This release intentionally raises Foghorn's minimum Helmsman control protocol from
v2 to v3 without a mixed-version window. The provider control plane upgrades before
the desired edge release is synchronized. During that window, existing v2 edges are
...
plane rollout. The same applies to a v2 edge that was offline during the rollout and
returns later. It cannot reconnect until it has been updated to v3. This one-time edge
