...
description: "Foghorn instances now talk to each other across clusters: peer telemetry over a persistent gRPC channel, cross-cluster viewer routing, and origin pulls that make many clusters behave like one CDN."
tags:
  - engineering
  - architecture
  - federation
...
As of the v0.1.0 cycle, Foghorn (the routing and balancing service that runs in every cluster) handles this itself. Foghorn instances in different clusters now exchange telemetry and arrange origin pulls, so any edge can serve any tenant's stream, regardless of which cluster the broadcaster pushed into.
...
Routing decisions land in analytics with the remote cluster marked, so you can see cross-cluster serves in the dashboard (they render amber, with a badge). That badge should be rare: the 200-point penalty makes local serving the strong default, and federation is the fallback that turns the worst case from "no stream" into a slightly farther edge.
