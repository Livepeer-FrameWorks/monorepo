...
We looked at the obvious alternative. Tailscale is excellent, and its coordination plane is a SaaS — topology visible to a third party and per-device pricing. Headscale trades that for a dependency on an external project at the center of our trust model. Provider mesh coordination therefore remains a first-party service: Privateer and Quartermaster.
...
Because the provider deployment models demand it. Shared and dedicated provider capacity use the same trust architecture. Customer-operated MistServer/Helmsman edges authenticate to the public Foghorn boundary and do not join the backend mesh, receive federation credentials, or operate their own FrameWorks CA. Customer-operated Foghorn and the coupled Livepeer, Chandler, object-storage, DNS, analytics, and control-plane services are outside the supported edge product.
