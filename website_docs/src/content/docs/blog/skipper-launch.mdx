...
Skipper runs periodic health checks on active streams. Every 30 minutes (configurable), it fetches stream health and client QoE metrics from Periscope, compares them against Welford running baselines it builds over time, and runs a deterministic triage:
...
Infrastructure monitoring checks node-level CPU, memory, and disk across all clusters, with persistence confirmation to prevent transient spikes from triggering alerts. Notifications go out via email, WebSocket, or MCP.

## Social Posting

Skipper can draft social media posts from platform events. The heartbeat agent and knowledge crawler push signals into an event collector — viewer records, bandwidth milestones, federation metrics, newly embedded documentation pages. A detector classifies and scores these signals against stored baselines.
...
3. **Your own agents** — connect Claude Code, OpenClaw, or any MCP client to the Gateway endpoint. Use `ask_consultant` for full-pipeline answers with confidence tagging alongside 28+ platform tools.
...
- **[Open Dashboard](https://chartroom.frameworks.network/skipper)** — start chatting
