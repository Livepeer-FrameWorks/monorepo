...
description: "Skipper searches curated streaming docs, runs diagnostics against your live streams, and manages resources through the same MCP gateway your own agents use. It also tells you when it's guessing."
tags:
  - announcements
  - skipper
  - ai
...
A question we see often, in some form, is "my viewers keep buffering, what's wrong?" No docs page answers that. Answering it means looking at the stream (rebuffer ratio, packet loss, which edges the viewers landed on, whether the encoder is keeping up) as well as the documentation.

Skipper does both. It's an AI video consultant built into FrameWorks: it searches curated streaming documentation, runs diagnostics against your live streams, generates API operations from the current schema, and, in authenticated dashboard chat, can manage streams and billing for you.

It connects through the same [MCP gateway](/agents/mcp) that external agents use. Same GraphQL API, same tool registry, same authorization checks. Skipper has no private access path into the platform; anything it can do, your own agents can do too. In the dashboard it operates with your authenticated account context. On the docs site it runs read-only. Over MCP, `ask_consultant` runs the full consultant pipeline with mutation tools blocked; agents that intend to change platform state call the dedicated Gateway tools, which carry their own authorization.
...
Ask "give me a Python script that creates a stream" and it introspects the live GraphQL schema, generates the operation, and wraps it in working code. The schema introspection happens at query time, so the output matches your current API version rather than an API shape from the model's training data.
...
And in authenticated dashboard chat it can act: create streams, refresh keys, clip the last 30 seconds of a broadcast, start DVR recording, upload VOD assets, check your balance, initiate supported crypto top-ups, or resolve x402 payment flows, all through the same GraphQL API you'd call directly.
...
- **Baseline deviations** — metrics beyond 2σ from the running mean are flagged, with a 5-sample minimum so warmup noise doesn't trigger an investigation.
