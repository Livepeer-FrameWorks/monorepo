---
title: "How Seven AI Agents Audited Our Streaming Platform"
date: 2026-01-31
authors:
  - frameworks
excerpt: "We ran a multi-agent audit process across our codebase and are turning it into a repeatable pipeline. Here's how it found race conditions, tenant isolation gaps, and data loss risks across 12 subsystems."
tags:
  - engineering
  - ai
  - architecture
...
We needed something repeatable that prioritizes what has not been reviewed recently and can move from findings to fixes. Not a one-time security audit. An ongoing process.

So we ran an audit process around seven AI-agent roles, then documented the automation needed to make it a continuous pipeline. Each role has a focused responsibility. They coordinate through files and pull requests — no shared memory, no message bus. Git is the orchestration layer.
...
In the planned continuous version, each cycle has the Sentinel select up to three subsystems to audit based on how long it has been since they were last reviewed. The entire flow from audit to merged PR passes through multiple independent agents, each operating on the structured output of the previous one.
...
We already use this pattern in production. Our Skipper monitoring agent runs on a 30-minute default heartbeat, checks stream health for eligible tenants with active streams, and makes a deterministic triage decision:
...
Each cycle iterates over eligible tenants and makes a per-tenant decision. Most of the time, the answer is `ok`. The agent logs `HEARTBEAT_OK` and goes back to sleep.
...
The planned audit pipeline applies the same pattern at a different scale: instead of checking stream health metrics every 30 minutes, it checks codebase staleness on a weekly cadence. Which subsystems have not been audited recently? Which have the highest risk weight? Only the stale, high-priority domains trigger an audit.

## Agent-by-agent breakdown

### Sentinel

The Sentinel will handle domain selection once the continuous pipeline is automated. Each subsystem has a staleness score based on when it was last audited and a priority weight reflecting its risk profile:
...
The scoring formula is `days_since_last_audit * priority_weight`. Billing gets a 1.5x multiplier because data loss means revenue impact. Stable subsystems like forms and ticketing get a 0.6x. The planned Sentinel picks the top three and dispatches them.

The Sentinel does not need to be an LLM agent. The decision is deterministic -- a staleness score does not benefit from reasoning. Save the LLM compute for the parts that need it.
...
- **File(s)**: `api_billing/internal/handlers` (example module)
...
We ran the manually orchestrated version of this process across all 12 subsystems in 26 batches of three tasks each. The numbers:
...
**Automated orchestration.** The domain staleness manifest, audit file convention, scripts, scheduled GitHub Actions workflow, and Makefile targets still need to be built before this is a continuous pipeline.
...
**Cross-subsystem audits.** The current audit plan is domain-oriented. The most interesting bugs live at the boundaries: what happens when a billing enforcement check races against an ingest signal that crosses through the analytics pipeline? Multi-domain audits would trace these cross-cutting flows.
...
**Incident-driven audits.** The runtime heartbeat agent already runs periodic investigations when deterministic triage says a tenant needs attention. A future audit pipeline could also consume incident or anomaly signals: when production telemetry flags something, schedule an audit of the affected domain rather than waiting for the next cycle.
