...
A **media cluster** is a set of edge nodes plus the cluster-scoped services
assigned to it, strictly the media plane. Every media cluster is served by a
platform-run Foghorn control cell (one Foghorn instance or HA set). One cell can
serve several virtual media clusters, including customer private clusters, so
the Foghorn process cluster is never a substitute for an edge node's cluster
identity. Commodore, Quartermaster, and Purser remain central. Today the
unsharded ClickHouse deployment also provides one logical Periscope metering
source. When ClickHouse is split by wider region, each logical regional
deployment runs local Periscope Ingest, Query, and Metering services; the
regional metering workers publish to the same central Purser through Kafka.
These services scale by logical data source, not one copy per media cluster.
...
| **Open Marketplace** | Operators publish listed clusters with access controls, invites, subscription requests, and pricing metadata. A vetted public marketplace program for third-party capacity is on the [roadmap](/roadmap). |
...
3. **Origin-Pull**: Foghorn arranges a DTSC pull from the remote origin into a local edge, and the viewer is served from there. Subsequent viewers are served locally. Viewers are never redirected to another cluster

Source advertisements carry both Mist's native buffer health state and a separate
media-playability signal. `FULL`, `DRY`, and `RECOVER` can all serve viewers; `DRY`
means the booted buffer has a diagnostic issue, not that it is empty. This distinction
matters for valid video-only streams, which Mist may flag with `HLSnoaudio!` while
continuing to serve video. Federation refuses only sources whose media is not playable.
...
2. **Forward (safety net)**: If the command arrives at a Foghorn that doesn't
   ...
   the artifact handles it. Each peer receives a fresh bounded attempt, so one
   unavailable peer does not consume the next peer's timeout window.
...
| `unlisted` | Anyone with the direct link | Via the billing checkout flow                    |
...
Tenant discovers cluster (marketplace UI or invite link)
  → Public marketplace: SubscribeToCluster routes the decision through Purser
  → Monthly: verified checkout webhook materializes active marketplace access
  → Custom/approval listing: Purser materializes an inactive PENDING_APPROVAL row
  → Cluster owner approves/rejects (ApproveClusterSubscription / RejectClusterSubscription)
  → Private invite: RequestClusterSubscription validates the invite in Quartermaster
  → Unlisted direct link: Purser validates pricing and materializes marketplace access
  → Tenant can now route streams through the cluster
```

Platform-official capacity is provisioned from the tenant billing tier, and a
tenant's own private edge cluster uses owner authority. Neither enters
marketplace checkout. v0.3 fails closed if a listing combines monthly checkout
with owner approval; that combination needs a future two-proof lifecycle.
...
See [DNS and Cluster Routing](/operators/dns) for details.

---

## Billing Attribution

Stream/viewer/artifact analytics carry `cluster_id` (the cluster that performed the work) and `origin_cluster_id` (where the stream was ingested, retained for audit). Delivery is always billed to the serving cluster; processing is billed to the executing cluster; storage is billed to its placement/provider cluster. Only truly tenant-wide meters such as API usage attach to the tenant's primary cluster.

- **Periscope Metering** generates per-cluster, dimensioned usage reports
...
- **Federation events**: All cross-cluster operations (peering, replication, artifact access) emit geo-enriched events to ClickHouse (`federation_events` table). View them on the **Federation** dashboard page (`/infrastructure/federation`)
- **Self-geo**: each Foghorn resolves its own coordinates from `infrastructure_nodes.external_ip` at bootstrap and stamps them on the events it emits. Peers do not exchange coordinates over the PeerChannel, so a remote position is left unset rather than guessed; the topology map sources remote coordinates from infrastructure records instead
