...
<Aside type="note" title="Marketplace status">
  Cluster listing, invites, subscription requests, approvals, preferred-cluster routing, and pricing
  model fields are implemented. A vetted public marketplace program still needs operator onboarding,
  published commercial terms, and support-boundary processes before third-party capacity should be
