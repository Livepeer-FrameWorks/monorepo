...
Foghorn instances are managed in a shared pool. Clusters can claim idle instances on creation, assign idle instances later, drain instances gracefully, or release existing assignments back to the pool.
...
frameworks admin foghorn-pool assign --cluster-id <CLUSTER_ID> --count 2
