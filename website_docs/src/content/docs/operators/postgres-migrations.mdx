...
<Aside type="caution" title="Rolling deploy safety">
  Apply migrations **before** deploying service versions that depend on new columns/tables. See
  [Running Cluster Upgrades](/operators/running-upgrades) for the full upgrade sequence.
</Aside>
...
└── v0.2.11/
    ...
    └── 002_crypto_wallet_status_and_quote.sql
```
...
```sql
-- Migration: v0.2.11/001_bootstrap_tenant_aliases.sql
-- Required before: quartermaster v0.2.11
