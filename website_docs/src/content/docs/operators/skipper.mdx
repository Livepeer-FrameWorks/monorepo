...
<Tabs>
  ...
  <TabItem label="API (Anthropic + OpenAI)">
    ...
    # Reranker — Cohere rerank (best quality, $2/1K queries)
  ...
  <TabItem label="OpenRouter">
    ...
    ```bash
    # Reranker — Jina (free tier available, strong multilingual)
...
| ----------------------------------- | ------------------------------------ | ------------------- | ---------------------------- |
| [Cohere](https://cohere.com/rerank) | `rerank-v3.5`                        | ~$2/1K queries      | Best proprietary quality     |
| [Jina](https://jina.ai/reranker)    | `jina-reranker-v2-base-multilingual` | Free tier available | Strong multilingual support  |
| Generic                             | Any `/v1/rerank` endpoint            | Varies              | Self-hosted BGE/MXBai models |
...
Skipper runs as the `api_consultant` service (ports 18018 HTTP, 19007 gRPC). It connects to the API Gateway via MCP to access platform tools (diagnostics, stream management, GraphQL). The Gateway proxies Skipper's `search_knowledge` and `search_web` tools to external MCP agents.
...
<Aside type="note" title="Prices as of February 2026">
