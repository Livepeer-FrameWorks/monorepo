...
<Aside type="caution" title="Estimates only">
  We do not have enough production Skipper usage data yet to publish reliable per-chat cost
  estimates. The table below is a planning aid, not a billing promise. We should replace it with
  measured data once we have real prompt sizes, tool-call counts, conversation lengths, provider
  choices, and model mix.
</Aside>
...
For budgeting, start by logging real `tokensInput`, `tokensOutput`, tool calls, and provider/model
per request. Recalculate the table from production data instead of maintaining static vendor
pricing in this page.
