...
Embedding models convert text into vectors for similarity search. Poor retrieval degrades every answer.
...
**Tier 2 — Recommended (14-32B)** — single 24-48GB GPU **(recommended)**
...
No local hardware needed. Skipper uses OpenAI-compatible chat and embedding endpoints, so hosted SaaS, [Blueclaw](https://blueclaw.network/), Livepeer-network inference, or a BYOC model container can all fit the same configuration shape when they expose compatible APIs.

For Livepeer-network deployments, check the current Livepeer AI docs and ask in the [Livepeer Discord](https://discord.com/invite/livepeer) before planning around a custom model or BYOC container. Availability changes as model runners and orchestrator support evolve. BYOC is useful when the model you want is not exposed by a managed endpoint yet.
...
<Aside type="caution" title="Estimates only">
  The numbers below are planning aids, not billing promises. Per-chat cost depends on prompt size,
  tool-call counts, conversation length, provider pricing, and model mix — measure your own
  deployment before committing to a support-tier margin.
</Aside>
...
| `LLM_CONTEXT_WINDOW`          | Chat model context window in tokens; `0` uses the known window for recognized models      | `0`              |
| `SKIPPER_PROMPT_TOKEN_BUDGET` | Explicit prompt token budget; `0` derives it from the context window and `LLM_MAX_TOKENS` | `0`              |
...
| `EMBEDDING_DIMENSIONS` | Embedding vector size; `0` probes the model at startup. Changing it truncates stored knowledge | `0`            |
...
| `SKIPPER_SSRF_ALLOWED_HOSTS`    | Comma-separated private hosts that the crawler may contact           | —       |
| `SKIPPER_CRAWL_ORIGIN_REWRITES` | JSON map of public origins to internal network origins for crawl I/O | —       |
...
| `SKIPPER_ADMIN_TENANT_ID`           | Tenant the embedded web UI acts as                                            | `local`                      |
| `SYSTEM_TENANT_ID`                  | System tenant that owns global/platform knowledge                             | reserved system tenant UUID  |
...
| `GATEWAY_MCP_URLS`                  | Comma-separated Bridge MCP endpoints; takes precedence over `GATEWAY_MCP_URL` | —                            |
| `GATEWAY_PUBLIC_URL`                | Public API Gateway base URL; fallback for MCP when neither MCP URL is set     | —                            |
| `LOOKOUT_GRPC_ADDR`                 | Lookout gRPC address for attaching investigation reports to incidents         | —                            |
| `KAFKA_BROKERS`                     | Aggregator Kafka brokers carrying the Lookout incident topic                  | —                            |

Lookout-triggered investigations run only when both `LOOKOUT_GRPC_ADDR` and `KAFKA_BROKERS` are set.
...
| `faq.txt`        | Curated local FAQ and troubleshooting articles            |

Set `SKIPPER_SITEMAPS_DIR` to the directory containing these files. In Docker Compose, this is mounted as a read-only volume at `/etc/skipper/sitemaps`. The `frameworks` CLI embeds these files and places them at `/etc/skipper` when it deploys Skipper, so no repository checkout is needed on the operator machine.
...
A local development stack with proper health checks and startup ordering:
...
| `SMTP_ALLOW_INSECURE`      | Send to an SMTP server without STARTTLS; isolated dev/test relays only, never production           | `false`                      |
| `FROM_EMAIL`               | Sender email address                                                                               | `noreply@frameworks.network` |
| `FROM_NAME`                | Sender display name                                                                                | `FrameWorks`                 |
| `TO_EMAIL`                 | Fallback recipient for heartbeat reports and infrastructure alerts when no tenant contact resolves | —                            |
| `SKIPPER_NOTIFY_EMAIL`     | Default email delivery of heartbeat reports when a tenant has no preference                        | `false`                      |
| `SKIPPER_NOTIFY_WEBSOCKET` | Default websocket delivery of heartbeat reports when a tenant has no preference                    | `true`                       |
| `SKIPPER_NOTIFY_MCP`       | Default MCP delivery of heartbeat reports when a tenant has no preference                          | `true`                       |
