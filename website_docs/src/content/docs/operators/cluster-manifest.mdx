...
- `allow_private_pull_sources` (optional) - Allow pull-input streams on this media cluster to use private literal source addresses such as RFC1918, ULA, or non-link-local multicast. Keep this `false` for platform-managed clusters and enable it only on self-hosted edges that can reach those sources.
