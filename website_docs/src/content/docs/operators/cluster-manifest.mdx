...
clusters:
  prod-control:
    ...
    type: central
    roles: [control, support, infra, mesh, interface, observability]
  prod-media:
    name: Production Media Plane
    type: central
    region: us-east
    roles: [routing, analytics]
```
...
**Role-based resolution:** When a service doesn't specify `cluster`, the CLI matches the service's role (from the built-in service registry) against each cluster's `roles` list. For example, `foghorn` has role `routing`, so it resolves to whichever cluster lists `routing` in its roles.
