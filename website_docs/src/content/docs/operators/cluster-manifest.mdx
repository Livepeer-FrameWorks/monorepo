...
The CLI fetches `cluster.yaml` (or whichever file `--manifest` specifies) from the repo, along with any referenced files like `env_file`. The standard provision pipeline then runs with the fetched data.
