...
- `NAVIGATOR_ALLOW_INSECURE=false`
- `DECKLOG_ALLOW_INSECURE=false`
- `DECKLOG_USE_TLS=true`

If `navigator` is enabled in a production manifest, the shared `env_file` must also provide:
...
fetch_cert: true
telemetry:
  remote_write_url: https://telemetry.example.com/api/v1/write
  username: edge-ingest
  password: "<edge-remote-write-password>"
nodes:
  - name: edge-us-east-1
...
- `telemetry` (optional) - remote_write configuration for per-edge vmagent
- `nodes` (required)
...
The CLI fetches `cluster.yaml` (or whichever file `--manifest` specifies) from the repo, along with all referenced `env_file` paths (top-level and per-service). The standard provision pipeline then runs with the fetched data.
