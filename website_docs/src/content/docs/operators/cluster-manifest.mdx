...
The cluster manifest is the YAML file the CLI reads to provision hosts, services, and interfaces.

<Aside type="tip">
  FrameWorks uses a GitOps-friendly approach. We recommend committing your cluster manifest to a
...
type: cluster
profile: production
channel: stable # optional: stable (default), candidate, or legacy rc
...
tls_bundles: {}
ingress_sites: {}
webhooks: {} # optional: destination policy for tenant webhooks
```
...
**Values:** `stable` (latest production-ready release), `candidate` (newest release,
including an RC), and legacy `rc` (newest RC only). Use `candidate` for staging and
release-like development environments.
...
Operator-declared `commodore.pull_streams` and `commodore.mist_native_streams` in the overlay
declare where their source may run with `source_location`:

```yaml
commodore:
  pull_streams:
    - playback_id: warehouse-cam
      owner_tenant: { ref: quartermaster.tenants[acme] }
      title: Warehouse Camera
      source_uri_ref: { sops: gitops/secrets/acme.env, key: WAREHOUSE_CAM_URI }
      enabled: true
      source_location:
        clusters: [warehouse-edge] # cluster keys from this manifest
        nodes: [warehouse-edge-1] # optional: host keys inside a listed cluster
        avoid_nodes: [warehouse-edge-3] # optional: hosts the source must never run on
  mist_native_streams:
    - playback_id: demo
      owner_tenant: { ref: quartermaster.system_tenant }
      # source, source_kind, always_on, ...
      source_location:
        clusters: [media-eu-1] # exactly one cluster
```

- Omit `source_location` to let a public pull source run on any media cluster. A private or
  multicast `source_uri` requires `clusters`, each with `allow_private_pull_sources: true`.
- `nodes` and `avoid_nodes` require `clusters`; every entry must be a host of a listed cluster, and
  a host cannot appear in both. Listing nodes narrows only that host's cluster.
- `mist_native_streams` require exactly one cluster.
- The previous `allowed_cluster_ids` key is rejected with an error naming `source_location`.
  Replace `allowed_cluster_ids: [a, b]` with `source_location: { clusters: [a, b] }`.

`commodore bootstrap` writes each location as the stream's own ingest placement rules and reports
`stream source_locations: updated=N noop=N`. Streams declared in the overlay take their location
from the overlay: a change made to such a stream's ingest rules through the API is replaced on the
next bootstrap run. A location that lists `nodes` or `avoid_nodes` needs every media cell serving
the stream's owner to support node placement: until then `commodore bootstrap` (and its
`--dry-run`) exits with an error before writing anything. Upgrade Foghorn in every cell, then rerun
bootstrap. See
[media placement](https://github.com/Livepeer-FrameWorks/monorepo/blob/master/docs/architecture/media-placement-policy.md#source-location).
...
clusters:
  ...
  prod-media:
    ...
    cell: media-eu
    class: platform
    control_cell: media-eu
    eligible_serving_cells: [media-eu]
    default: true
...
- `cell` (optional) - Stable media-cell identity. Defaults to the cluster ID.
- `class` (optional) - Placement class used when grouping compatible media cells.
- `control_cell` (optional) - Foghorn cell that owns this cluster's control-plane projection. Defaults to `cell`.
- `eligible_serving_cells` (optional) - Explicit Foghorn cells allowed to receive and serve this cluster's signed media authority. Defaults to `[control_cell]`. Every listed cell must contain an enabled Foghorn deployment; production rendering fails instead of silently omitting an unknown cell.
...
- `allow_private_pull_sources` (optional) - Capability flag allowing pull-input streams on this media cluster to use private literal source addresses such as RFC1918, ULA, or non-link-local multicast. Keep this `false` for platform-managed clusters and enable it only on self-hosted edges that can reach those sources. This does not place a pull stream by itself; a private source must also restrict its source location to consenting clusters (`source_location.clusters` in the bootstrap overlay, `sourceLocation` with mode `RESTRICTED` through the API).
- `public_topology` (optional) - Show this cluster on public map and coverage views.
- `env_files` (optional) - Env files merged into every cluster-scoped service replica for this cluster, after the top-level `env_files` and before per-service `env_file`. Use them for per-cluster credentials such as region-specific `STORAGE_S3_*` access keys. Paths are relative to the manifest directory; absolute paths are rejected. SOPS-encrypted files are decrypted during provisioning.
- `s3_bucket`, `s3_endpoint`, `s3_region`, `s3_prefix` (optional) - Artifact storage descriptor reconciled into the Quartermaster cluster row. When `s3_bucket` is set, `s3_prefix` must be set explicitly (use `s3_prefix: ""` for no prefix). Access keys stay in env files. See [Media storage](/operators/media-storage/).
...
Destination policy for every tenant-supplied webhook the platform dials:
outbound event webhooks (Bosun) and playback-auth webhooks (Commodore validates
the URL, Foghorn calls it).
...
- `allow_private_destinations` (default `false`) - lets webhook receivers use private
  (RFC 1918 / ULA) addresses, plain `http`, and `.local` or `.internal` host names, for an
  isolated staging or development cluster whose receivers live on its own network. Loopback,
  link-local, and cloud metadata addresses stay blocked. The CLI renders it as
  `BOSUN_ALLOW_PRIVATE_DESTINATIONS` on Bosun and `PLAYBACK_WEBHOOK_ALLOW_PRIVATE_DESTINATIONS`
  on Commodore and Foghorn; an env file or a service's `config` map still overrides it.
  Never enable it on a cluster that serves untrusted tenants.
  ...
  postgres:
    ...
    databases:
      - name: quartermaster
        ...
        runtime_role: quartermaster_runtime
```
...
- `instances` (optional) - Additional named vanilla PostgreSQL instances. Each entry takes `name`, `host`, and optional `port`, `version`, `mode`, `password`, `databases` (same shape as below), `tuning`, and `config`
- `port` (required)
- `replication_factor` (optional) - Defaults to the number of `nodes`
- `databases` (optional) - Each entry declares `name` and its schema/migration
  `owner`. Optional `runtime_role` opts service DSNs into the separately
  provisioned least-privilege login. It authenticates with
  `DATABASE_RUNTIME_PASSWORD`, never the owner credential. Provision the role,
  verify it with `cluster doctor --deep`, then enable the application cutover.
  Runtime secrets resolve in this order: `POSTGRES_<DATABASE>_RUNTIME_PASSWORD`,
  the equivalent owner-name alias, a named Postgres instance override, the
  matching cluster's `DATABASE_RUNTIME_PASSWORD`, then the shared
  `DATABASE_RUNTIME_PASSWORD`. The same resolution feeds provisioning and service
  DSNs, and provisioning rejects owner/runtime credential collisions.
  On YugabyteDB, table placement is declared in the repository rather than here:
  provisioning creates each platform database with its colocated layout, and
  existing databases move with
  [`frameworks cluster yugabyte relayout`](/operators/yugabyte-layout).
...
- `mode` (required) - `native`
- `version` (required)
- `nodes` (required, at least one) - Replicated cluster nodes as `{host, id}` entries. `host` is a key from `hosts`; `id` (1 or higher) drives task names, the Keeper server ID, and the replica name. Every node uses the cluster-level `port`. The removed `host` field is rejected with a message pointing to `nodes`.
- `read_endpoint` (optional) - Host key that `periscope-query` uses instead of `nodes`
- `write_endpoint` (optional) - Host key that `periscope-ingest` and `periscope-metering` use instead of `nodes`. Both endpoint overrides exist for [moving the ClickHouse node](/operators/clickhouse-migrations#moving-the-physical-clickhouse-node).
- `port` (required)
- `databases` (optional)
- `sql_access` (optional) - `direct` by default, or `ssh`
...
- `instances` (required)
  ...
  - `mode` (optional) - `single` (default) or `sentinel` for a primary with replicas and a Sentinel quorum
  - `host` (required) - Host name from `hosts`; the initial master in `sentinel` mode
  - `port` (required)
  - `password` (optional)
  - `cluster` (optional) - Limit the instance to consumers in this cluster; empty applies globally
  - `master_name` (optional) - Sentinel master name; defaults to `name`
  - `replica_hosts` (optional) - Sentinel replica hosts, excluding `host`
  - `sentinels` (optional) - Sentinel quorum members as `{host, port}` entries (`port` defaults to `26379`); consumers connect through these
...
infrastructure:
  kafka:
    ...
    topics:
      ...
      # ...plus every other platform topic in the table below
```
...
- `region_id` (optional) - Region of this Kafka cluster, for example `eu-west`. Empty infers it from broker host labels
- `role` (optional) - `aggregator` or `regional`. The top-level cluster defaults to `aggregator`
- `regional` (optional) - Additional Kafka clusters in other regions. Each entry is an independent KRaft deployment with `region_id` (required), `role` (default `regional`), `cluster_id`, and the same `controllers`, `controller_port`, `brokers`, `topics`, and replication settings as the top level
- `mirrormaker` (optional) - MirrorMaker2 links between Kafka regions; nothing is mirrored when absent or disabled
  - `enabled` (required)
  - `mode` (optional) - `native` (default)
  - `heap_opts` (optional) - JVM heap; default `-Xmx1G -Xms1G`
  - `replicas` (optional) - Replication factor for mirrored topics on the target; defaults to the target broker count
  - `task_count` (optional) - Default `tasks.max` for links that do not set one
  - `links` (required when enabled) - One entry per ordered pair of regions, each with `source` and `target` region IDs, `hosts` (worker hosts, all in the target region), optional `topics` (empty uses the canonical set for that direction), and optional `task_count`
...
- `topics` (required) - every topic must set `config.retention.ms`. Each Kafka cluster must declare the platform topics below: the aggregator cluster all of them, and each `regional` cluster all except `lookout.incidents`. The platform topics have fixed settings, and the CLI refuses a manifest that omits one or whose values differ, printing the entry to add:

  | Topic                         | `retention.ms`           | Other required settings                                                                            |
  | ----------------------------- | ------------------------ | -------------------------------------------------------------------------------------------------- |
  | `analytics_events`            | `604800000` (7 days)     |                                                                                                    |
  | `service_events`              | `15552000000` (180 days) |                                                                                                    |
  | `analytics.raw_mist_triggers` | `2592000000` (30 days)   |                                                                                                    |
  | `billing.usage_reports`       | `31536000000` (365 days) |                                                                                                    |
  | `decklog_events_dlq`          | `7776000000` (90 days)   |                                                                                                    |
  | `lookout.incidents`           | `604800000` (7 days)     |                                                                                                    |
  | `domain.events`               | `15552000000` (180 days) | `partitions: 12`, `replication_factor: 3` (or the broker count if lower), `cleanup.policy: delete` |

  Provisioning creates missing topics and applies the declared config to topics that already exist, without restarting brokers.
...
- `clusters` (optional) - List form of `cluster` for cluster-scoped media services that serve several clusters. Cannot be combined with `cluster`. Foghorn and Chandler deployments must declare exactly one cluster; use one service entry per cell
...
- `depends_on` (optional)
- `public` (optional) - Marks the service as having a public-facing endpoint
- `update_strategy` (optional) - Rollout override for `cluster apply`: `max_unavailable`, `canary`, `region_stagger`, `primary_last`
...
nodes:
  - name: edge-us-east-1
    ...
    region: us-east
    # Private control/telemetry routing while retaining public certificate names.
    foghorn_addr: 10.20.0.11:18029
    foghorn_tls_server_name: foghorn.media-eu.example.com
    telemetry_address: 10.20.0.11
    apply_tune: true
    register_qm: true
```
...
- `channel` (optional) - `stable`, `candidate`, legacy `rc`, or an explicit version such as `v0.2.0-rc3`
...
- `mode` (optional) - Default node mode: `container` (default; single edge image) or `native`. `docker` is a deprecated alias for `container`.
...
Edge telemetry ingress is control-plane managed. Enrolled edge nodes receive remote-write URL and auth material from Foghorn during preregistration and subsequent `ConfigSeed` updates. For a private staging network, `telemetry_address` pins only the address lookup; vmagent retains the URL hostname for HTTP routing, TLS SNI, and certificate verification.
...
- `subdomain` (optional)
- `region` (optional)
- `cluster` (optional) - Per-node cluster override; useful when one edge manifest spans multiple media cells.
- `foghorn_addr` (optional) - Private or otherwise explicit Foghorn gRPC endpoint.
- `foghorn_tls_server_name` (optional) - Certificate name to verify when `foghorn_addr` is an IP or private alias.
- `telemetry_address` (optional) - Private IP for the control-plane-derived telemetry hostname. This does not disable TLS verification or change the HTTP hostname.
- `apply_tune` (optional)
- `register_qm` (optional)
- `mode` (optional) - Per-node override, `container` or `native` (`docker` is a deprecated alias for `container`)
