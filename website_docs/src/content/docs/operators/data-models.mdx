...
| `foghorn_cluster_assignments` | Many-to-many Foghorn ↔ cluster mapping.              |
...
| `billing_tiers`          | Plan definitions, limits, and default tier flags (`is_default_prepaid`, `is_default_postpaid`). |
...
Current state snapshots, deduplicated by primary key. Check the ClickHouse bootstrap schema for the exact engine semantics (ReplacingMergeTree vs AggregatingMergeTree).
...
| Table                      | Description                                            | Cluster Columns                   |
| -------------------------- | ------------------------------------------------------ | --------------------------------- |
| `stream_event_log`         | Lifecycle events (start, end, buffer) and push status. | `cluster_id`, `origin_cluster_id` |
| `viewer_connection_events` | Viewer connect/disconnect sessions.                    | `cluster_id`, `origin_cluster_id` |
...
Background processes that roll up raw data into optimized summaries. See the ClickHouse bootstrap schema for the full list.
...
Billing-critical MVs (`viewer_hours_hourly`, `tenant_viewer_daily`) include `cluster_id` and `origin_cluster_id` in their GROUP BY and ORDER BY keys. This prevents merge-collapse in SummingMergeTree engines and enables per-cluster billing attribution. See `docs/architecture/cross-cluster-billing.md` for the settlement query.
