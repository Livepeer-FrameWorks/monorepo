...
| Bosun               | Tenant webhook endpoints and signing secrets, received domain events, deliveries, and delivery attempts.    | `pkg/database/sql/schema/bosun.sql`                                            |
| Lookout             | Incidents built from Alertmanager notifications, their alerts and timeline, and the notification outbox.    | `pkg/database/sql/schema/lookout.sql`                                          |
...
### Purser small-balance collection state

`billing_collection_balances` is the lockable tenant/currency state for
provider-collected postpaid overages below the 500-cent economic floor.
`billing_collection_entries` records every invoice decision and its opening,
current, collected, and closing cents. `billing_collection_writeoffs` records
an explicit account-closure or operator-approved waiver. These are financial
ledgers owned by Purser; integrations read the invoice API and must not mutate
the tables directly.

## Schema Sources

This page is intentionally not a full table catalog. The repository SQL is the contract used by the
CLI/provisioner and local development stack.
...
| Billing tier catalog                     | `api_billing/internal/bootstrap/catalog/billing_tiers.yaml` | Embedded in the Purser binary. GitOps adds tiers or overrides fields through `purser.billing_tiers` in the bootstrap file. |
| Demo seed data                           | `pkg/database/sql/seeds/demo/`                              | Local/demo data only; separate from demo-mode API fallbacks.                                                               |
...
- **Canonical ledgers**: 5-minute usage ledgers used by regional metering where
  the meter contract calls for them. Finalized session/processing facts and
  canonical storage snapshots remain direct billing inputs.
- **Dashboard rollups**: hourly and daily aggregates used for dashboards and
  capacity reporting. Billing never reads these caches.

Canonical facts and ledgers retain the work `cluster_id`; viewer facts also
retain `origin_cluster_id` for lineage. See
`docs/architecture/cross-cluster-billing.md` for the settlement model.
...
<Aside type="tip">
  Raw IPs remain in ClickHouse for future per-tenant analytics features. Platform APIs enforce
  redaction.
</Aside>
