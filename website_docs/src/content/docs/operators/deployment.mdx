...
This guide covers a full operator-owned deployment. If you only want to add customer-operated media
capacity to the managed FrameWorks control plane, use the [hybrid edge guide](/hybrid/overview).
...
Authority work queues use range-sharded due-time indexes on YugabyteDB. If Commodore repeatedly logs
`Failed to observe media authority delivery backlog` or `Failed to claim short-lease media authority
delivery`, verify the v0.3.10 expand and postdeploy migrations before increasing worker timeouts. The
normal retention jobs preserve all current and retryable authority while bounding completed inbox rows
and expired terminal history; operators should not delete those tables manually.
