...
`--ready` extends the pipeline with `cluster init` and static reference seeds
(`cluster seed`), plus optional admin bootstrap if you pass
`--bootstrap-admin-email`. For PostgreSQL/YugabyteDB, `cluster init` applies
baseline schemas first and then embedded `expand` migrations from the
database/version/phase migration layout. Add `--seed-demo` to also load sample
tenant/user/stream data.
...
- The Gateway has tenant-aware rate-limit middleware, but current free-tier product limits are not commercially enforced yet.
