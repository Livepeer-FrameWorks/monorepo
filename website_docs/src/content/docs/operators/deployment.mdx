...
# Copy config/env/secrets.env.example → gitops-private/secrets/production.env

# 4. Detect hosts (verify SSH connectivity)
frameworks cluster detect --manifest cluster.yaml

# 5. Provision everything (infrastructure → QM → mesh → services → interfaces)
...
**GitOps Deployment (from a private GitHub repo):**

If your manifests are in a private repo managed by a GitHub App:
...
frameworks config set github.repo org/infra-repo

# Provision from the repo (fetches cluster.yaml, .env, etc.)
frameworks cluster provision --repo org/infra-repo

# If env files are SOPS-encrypted (recommended), specify an age key:
frameworks cluster provision --repo org/infra-repo --age-key ~/.config/sops/age/keys.txt
```

This fetches the cluster manifest and referenced files from the repo, then runs the standard provision pipeline. SOPS-encrypted env files are decrypted automatically if an age key is available (see [External Services](/operators/external-services#encrypting-secrets-with-sops--age)). Operators manage config changes via pull requests.

<Aside type="note" title="Provisioning Phases">
The CLI provisions in dependency order:

1. **Infrastructure** — Postgres, Kafka, Zookeeper, ClickHouse (parallel)
