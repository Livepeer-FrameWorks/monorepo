...
This guide is for operators who cannot or choose not to use the automated CLI deployment. For most users, we recommend `frameworks cluster provision` instead. See [CLI Reference](/operators/cli-reference).
</Aside>
...
<Aside type="tip">
For streaming protocols (WebRTC, RTMP, SRT), use DNS-only mode (not proxied) to avoid protocol interference.
</Aside>
...
<Aside type="note">
Chatwoot does not support webhook HMAC signature verification.
...
<Aside type="caution">
Edge nodes are **not** on the mesh. Prefer TLS for Decklog and Foghorn control when exposed publicly.
</Aside>
...
| RECORDING_* | `http://localhost:18007/triggers` | Clip/DVR lifecycle |
...
<Aside type="note">
Navigator is optional for manual deployments. If you prefer to manage DNS records manually (as shown in Phase 1), skip this section.
</Aside>
...
docker run -d --name frameworks-webapp \
...
docker build -f website_marketing/Dockerfile \
  ...
  # ... (all VITE_* args)
  -t frameworks-marketing .

docker run -d --name frameworks-marketing \
  --restart unless-stopped \
  -p 18031:18031 \
...
docker run -d --name frameworks-docs \
  --restart unless-stopped \
  -p 18033:18033 \
  frameworks-docs
```
...
```bash
# /etc/systemd/system/frameworks-webapp.service
[Unit]
Description=FrameWorks Web Console
...
Restart=always
RestartSec=5
ExecStartPre=-/usr/bin/docker stop frameworks-webapp
ExecStartPre=-/usr/bin/docker rm frameworks-webapp
ExecStart=/usr/bin/docker run --rm --name frameworks-webapp -p 18030:18030 frameworks-webapp
ExecStop=/usr/bin/docker stop frameworks-webapp
...
<Aside type="tip">
For streaming endpoints (`edge.your-domain.com`, `ingest.your-domain.com`), use DNS-only mode without proxying to avoid interference with RTMP, SRT, and WebRTC protocols.
</Aside>
