...
| Type    | Name        | Content                  | Notes              |
| ------- | ----------- | ------------------------ | ------------------ |
| A/CNAME | edge-egress | Load balancer or edge IP | Edge node delivery |
...
| A    | `edge-egress.{slug}.your-domain.com`    | Edge node IP(s)    | Cluster-scoped delivery    |
...
VITE_STREAMING_EDGE_URL=https://edge-egress.your-domain.com
...
STREAMING_EDGE_URL=https://edge-egress.your-domain.com
...
<Aside type="tip">
  For streaming endpoints (`edge-egress.your-domain.com`, `edge-ingest.your-domain.com`), use
  DNS-only mode without proxying to avoid interference with RTMP, SRT, and WebRTC protocols.
</Aside>
