...
<Aside type="note" title="Native edge nodes auto-update">
  Use native mode for production edge nodes when you want release-target reconciliation. Foghorn
  currently auto-updates native Helmsman, MistServer, and Caddy components over the Helmsman control
  stream; MistServer binary updates use an in-place controller reload. Docker edge nodes do not
  auto-update yet, even though they still report component versions. Update Docker edge nodes
  manually with `frameworks edge update` or your compose rollout process.
</Aside>
