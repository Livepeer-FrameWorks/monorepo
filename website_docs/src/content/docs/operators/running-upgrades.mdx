...
until it is upgraded. **Redeploy every edge after** the control plane, and those nodes
reconnect cleanly. When a release advances the minimum with no mixed-version window, this
is a hard cut — the specific minimum and any transitional guidance ride in that release's
notes, not here.
...
- **Redeploy every edge afterward.** A not-yet-upgraded (or freshly restarted,
  not-yet-reported) sidecar keeps streaming live, but is rejected at registration if
  below the minimum, or cordoned for stored artifacts until it sends its first
  versioned snapshot.
