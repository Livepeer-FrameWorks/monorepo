...
**Required Provider:** Cloudflare for root/API/web/admin DNS. Bunny DNS is optional; when `BUNNY_API_KEY` is configured, Navigator delegates media cluster zones to Bunny.
...
Navigator automates DNS record management based on inventory. Cloudflare remains the root/API/web/admin DNS provider, while Bunny DNS handles delegated media cluster zones. By default, Cloudflare proxying is enabled for `bridge`, `chartroom`, `chatwoot`, `foredeck`, `grafana`, `listmonk`, `logbook`, `metabase`, and `steward`; set `NAVIGATOR_PROXY_SERVICES` to a comma-separated list of service types if you need different proxy behavior. See [DNS Automation](/operators/dns) for details.
