...
| ---------------------- | ------------------------------------------------------------- | ------------------------------ |
| **Operator-provided**  | `DATABASE_PASSWORD`, `CLICKHOUSE_PASSWORD`, Stripe keys, SMTP | You — in your env_file         |
...
Auto-generated secrets use `crypto/rand` (64-char hex). The CLI generates them once and they persist across re-provisions via your env_file.

### Infrastructure Credential Flow

During `frameworks cluster provision`, the CLI also provisions database users:

- **Postgres/YugabyteDB**: Creates the `DATABASE_USER` role (from your env_file) with `DATABASE_PASSWORD`, grants access to all configured databases
...
1. Copy the template: `cp config/env/secrets.env.example gitops/secrets/production.env`
2. Set at minimum: `DATABASE_PASSWORD`, `CLICKHOUSE_PASSWORD` (security secrets can be left as `change-me`)
3. Reference it in your cluster manifest: `env_file: secrets/production.env`
4. Run `frameworks cluster provision` — the CLI creates database users, generates missing secrets, and pushes merged env vars to all hosts
