...
Navigator automates DNS record management based on inventory. By default, Cloudflare proxying is enabled for `bridge`, `chandler`, `chartroom`, `chatwoot`, `foredeck`, `listmonk`, `logbook`, and `steward`; set `NAVIGATOR_PROXY_SERVICES` to a comma-separated list of service types if you need different proxy behavior. See [DNS Automation](/operators/dns) for details.
