...
| DNS            | Cloudflare (required) + Bunny DNS (optional, recommended) | Yes                             | Navigator (api_dns)                                      |
| SSL/TLS        | Let's Encrypt + Google Trust Services (multi-CA)          | Yes                             | Navigator (api_dns)                                      |
| Object Storage | Cloudflare R2 / Hetzner / any S3-compat                   | Yes                             | Operator (bucket + CORS); Chandler reads per-cluster row |
