...
External service credentials required by the provider-operated platform today. Architecture work may reduce this surface with native Ceph-backed storage and provider-operated Anycast DNS; it is not a customer whole-stack hosting roadmap.
...
3. Set at minimum in `gitops/secrets/production.env`: distinct `DATABASE_PASSWORD` and `DATABASE_RUNTIME_PASSWORD` values, `CLICKHOUSE_PASSWORD`, `SERVICE_TOKEN`, `JWT_SECRET`, `PASSWORD_RESET_SECRET`, `FIELD_ENCRYPTION_KEY`, `USAGE_HASH_SECRET`, and a shared 64-hex-character `TELEMETRY_TOKEN_SECRET`
...
- **Purser** (`api_billing`) — records the payer country on crypto top-up invoices

If `GEOIP_MMDB_PATH` is empty, GeoIP is disabled and services fall back to non-geographic behavior. A service whose `GEOIP_MMDB_PATH` is set to a file that is missing or cannot be opened refuses to start.
