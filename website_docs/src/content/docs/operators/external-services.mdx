...
- **Postgres/YugabyteDB**: Creates a per-database owner/migration role plus a least-privilege runtime role (by default `<owner>_runtime`) using `DATABASE_PASSWORD`. The runtime role can connect and perform application DML in its service schema, but cannot create schema objects or own them. Add `runtime_role` to the database manifest entry only after provisioning and `frameworks cluster doctor --deep` succeed; omitting it keeps existing services on the owner role during a rolling upgrade.
...
3. Set at minimum in `gitops/secrets/production.env`: `DATABASE_PASSWORD`, `CLICKHOUSE_PASSWORD`, `SERVICE_TOKEN`, `JWT_SECRET`, `PASSWORD_RESET_SECRET`, `FIELD_ENCRYPTION_KEY`, `USAGE_HASH_SECRET`, and a shared 64-hex-character `TELEMETRY_TOKEN_SECRET`
