...
Navigator handles ACME certificate issuance and renewal via DNS-01. See
[DNS and Cluster Routing](/operators/dns) for certificate routing and provider
ownership. Set `NAVIGATOR_CERT_ALLOWED_SUFFIXES` when a deployment should
restrict certificate requests beyond the manifest `root_domain`.
