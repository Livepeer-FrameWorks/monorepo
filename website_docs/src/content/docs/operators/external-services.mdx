...
Navigator automates DNS record management based on inventory. By default, Cloudflare proxying is enabled for `chartroom`, `foredeck`, and `logbook`; set `NAVIGATOR_PROXY_SERVICES` to a comma-separated list of service types if you need different proxy behavior. See [DNS Automation](/operators/dns) for details.
