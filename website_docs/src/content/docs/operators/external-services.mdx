...
- **Operator analytics (optional)**: When `ANALYTICS_RO_PASSWORD` and `CLICKHOUSE_ANALYTICS_PASSWORD` are both set, provisioning additionally creates the read-only `frameworks_analytics_ro` Postgres role (quartermaster/commodore/purser grants, applied by `cluster seed`), the ClickHouse named collections that reach it, and a dedicated `frameworks_analytics` ClickHouse user. Use `frameworks_analytics` as the Metabase datasource credential for the tenant-activity cards (`infrastructure/metabase/activity_cards.yaml`) — the runtime `frameworks` user is deliberately denied the Postgres reach-through.
