...
Backups are local-only. The command writes timestamped artifacts under the
selected output directory; treat the exact filenames as CLI-owned.
...
- `postgres` - Restore from SQL dump. Every restored Foghorn database gets its
  media-authority restore fence raised: the cell decides on its restored media
  authority only after Commodore has confirmed or re-sent it, and cannot serve
  it while Commodore is unreachable. The fence lowers by itself once the cell
  and Commodore agree on what it holds.
...
steps or clear a fence to bypass an unreachable core. The `postgres` dump restore
command only supports Docker PostgreSQL; it refuses native PostgreSQL and
Yugabyte before stopping services. Use the fenced native workflow above for
current native deployments.
