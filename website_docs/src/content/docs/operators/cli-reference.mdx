...
3. **Cluster Registration:** A cluster record is created in Quartermaster with:
   - **Cluster ID:** Auto-generated as `{manifest.Type}-{manifest.Profile}` (e.g., `cluster-production`)
   - **Cluster Name:** Derived from manifest type/profile
   - **Cluster Type:** From manifest (`cluster` or `edge`)
   - **Base URL:** Derived from Bridge if present; otherwise Quartermaster (`http://<host>:<port>`)
...
- `--ssh-key` - SSH private key path
- `--pool-domain string` - Shared LB pool domain (e.g., edge-egress.example.com)
...
- `svc-<service>.yml` fragments (one per service)
