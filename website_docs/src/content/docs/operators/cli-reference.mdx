...
Run a full cluster release as one ordered, resumable plan: expand migrations → service
upgrades in dependency order → reconciliation transitions interleaved at their declared
dependency points → postdeploy migrations. Contract migrations are printed as a deferred
...
runs database, artifact, health, and reconciliation checks against the cluster.
