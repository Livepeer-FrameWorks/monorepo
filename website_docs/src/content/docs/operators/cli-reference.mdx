...
| Command                                                 | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `frameworks cluster finalize`                           | Resume service-owned post-provision finalization without provisioning or restarting services; supports `--only quartermaster`, `--only purser`, `--only commodore`, `--only assignments`, and `--only validation`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
dependency points → postdeploy migrations. This is the **normal** upgrade path;
...
```bash
# Preview the whole release (migrations, reconciliations, upgrades)
...
Inspect and adopt the immutable S3 storage descriptor a media cell is bound to.
...
```bash
frameworks cluster storage adopt <cluster> [flags]
```

`frameworks cluster storage adopt` brings a cell whose backend lives only in
`STORAGE_S3_*` env files, with nothing on the Quartermaster cluster row, under the
authority. It reads the cell's **deployed** descriptor, requires the cell's
replicas to agree, records it on the Quartermaster cluster row, and verifies it.
Quartermaster enforces immutability: an unset descriptor is populated, a matching
one is idempotent, and a diverging one is refused as a repoint. Credentials are
never touched.

**Flags:**

- `--dry-run` - Read the deployed descriptor and report it without writing Quartermaster

**Examples:**

```bash
# Read the live replicas and show the descriptor without writing
frameworks cluster storage adopt media-us --dry-run

# Reconcile the deployed backend into Quartermaster and verify
