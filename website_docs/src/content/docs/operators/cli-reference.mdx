...
| `frameworks cluster migrate`               | Apply pending SQL migrations from `pkg/database/sql/migrations/`; use `--dry-run` for Ansible check/diff mode.                         |
...
<Aside type="caution" title="Database schema changes">
  ...
  `frameworks cluster migrate` before upgrading services that depend on the new schema.
</Aside>
