...
| `selfhosted` | Public Bridge plus saved Quartermaster/Foghorn gRPC endpoints for an owned edge/self-hosted footprint. It does not imply access to the platform core. | `edge *`, `cluster nodes *` for owned clusters                                 |
...
For platform contexts, per-service non-local URLs saved with `frameworks context set-url` still override manifest-derived targets, but they must declare transport explicitly: use `--tls` for TLS endpoints or `--allow-insecure` for plaintext gRPC. Localhost defaults, manifest SSH tunnels, and mesh-derived addresses keep their established transport behavior. Localhost defaults are ignored in `ssh` and `mesh` modes so a newly created platform context does not accidentally dial the operator laptop for core services.
...
- `--ready` - After service batches, also run `cluster init` + `cluster seed` (static seeds) so the platform is usable in one command.
- `--seed-demo` - With `--ready`, additionally apply demo seeds (sample tenant/user/stream).
...
```bash
# Provision + init + seeds + admin bootstrap in one shot
frameworks cluster provision --ready \
  --bootstrap-admin-email you@co --bootstrap-admin-password-env DEPLOY_PW

# Provision entire cluster (no init/seed)
...
Use `frameworks cluster nodes ...` for edge-node lifecycle operations in clusters
you operate. Platform/provider contexts use GitOps/service-token backed
control-plane access. Self-hosted owner contexts use the active tenant JWT
against configured owner control-plane endpoints. They default to the active
context's `cluster_id`; when a TTY has no active cluster, the CLI prompts for
one. Pass `--cluster-id` when scripting against another cluster.
...
`add` expands an existing cluster. It uses the active context's `cluster_id` or
prompts for a cluster on TTYs, mints a short-lived enrollment token internally
through Quartermaster, passes it into the edge deployment pipeline, and does not
print the token. Platform contexts mint with service auth and the system tenant;
self-hosted owner contexts mint with the active tenant JWT. Quartermaster
requires owner/operator/provider authority; ordinary cluster subscriptions
cannot mint node enrollment tokens. Lifecycle-managed adds default to native
mode with the stable manifest so release-target updates can swap native
artifacts. Pass `--mode docker` only for nodes managed outside the agent-pull
release reconciler.
...
Manage the edge release catalog and the active cluster's rollout target.
...
Quartermaster. Use `--version` for the GitOps release version or channel; `--os`
and `--arch` select the component artifacts recorded in the release. Use
`--security` or `--metadata security=true`/`--metadata kind=security` when the
release should satisfy `security_only` rollout policy. GitOps manifests can also
carry `security: true`, `kind: security`, `release_kind: security`, or a
`metadata:` map. This is the provider repair/backfill path. Provider contexts
also publish the selected GitOps manifest automatically when setting a release
target; owner contexts set targets against releases already present in the
catalog. Release rows must include at least one updateable edge component
(`helmsman`, `mist`, or `caddy`); config-schema-only rows are rejected because
runtime configuration still flows through ConfigSeed.

### frameworks cluster releases target

Manage a cluster's desired edge release target.
...
Set the active cluster's target channel, optional pinned version, rollout policy,
and rollout-plan JSON. Pass `--cluster-id` when scripting. Foghorn reconciles
non-manual targets against reported node component versions: Helmsman and Caddy
changes apply over the existing Helmsman control stream, while Mist changes go
through cordon, drain, update, warmup version confirmation, endpoint probing,
and return-to-normal. Config-schema versions are reported for visibility; actual
runtime config still flows through ConfigSeed. Provider contexts can pass
`--security` or `--metadata key=value` here too; those flags apply to the
automatic publish performed before the target is saved.
...
Deploy an edge node in one command. Handles private cluster setup, enrollment token generation, pre-registration, and provisioning.

**Mode A — Logged-in tenant (automatic VPC setup):**

```bash
# Deploys to your private cluster (creates one if needed)
...
- `--cluster-name` - Name for new private cluster if one needs to be created
- `--region` - Region for new private cluster
- `--enrollment-token` - Pre-existing enrollment token (skips login/VPC setup)
...
`--register` is for the manual provisioning path, where an admin supplies `--pool-domain`/`--node-domain` directly without an enrollment token and wants to create the Quartermaster node row in the same step. It is rejected when combined with `--enrollment-token` (the token path already registers the node via Foghorn's `PreRegisterEdge`) and when the active context uses the `user` persona (which intentionally does not have Quartermaster reachability — only Bridge). Manifest entries that set `register_qm: true` together with an enrollment token are rejected the same way.

**Examples:**

```bash
# Provision single node with explicit domains (manual/admin path; requires
# a platform or self-hosted context with Quartermaster reachability)
...
Run the interactive onboarding wizard. It captures the operator persona,
Bridge/control-plane endpoints, and GitOps manifest defaults into a CLI
context, then makes that context current. Self-hosted setup asks for
Quartermaster and Foghorn gRPC addresses because `cluster nodes list`,
`drain`, `remove`, and `evict` call owner control-plane APIs rather than
platform GitOps service tokens.
