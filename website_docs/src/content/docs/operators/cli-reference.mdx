...
The FrameWorks CLI is your swiss-army knife for platform management. From provisioning entire clusters to diagnosing network mesh health, this tool wraps complex Ansible automation into simple, idempotent commands.

<Aside type="tip">
Use `frameworks menu` to launch an interactive TUI for exploring these commands.
</Aside>
...
Note: The `--manifest` flag is specific to cluster subcommands, not a global flag.
...
**Checks:**
- **Infrastructure:** Postgres, Kafka, Zookeeper, ClickHouse connectivity and query performance
- **Applications:** Health endpoints, database connectivity, Kafka consumer status
- **Networking:** Service reachability, WireGuard mesh (if configured)
- **Data:** Schema validation, table existence
...
**Process:**
1. Detect deployment mode from GitOps manifest
2. Generate configuration files (docker-compose.yml or systemd units)
3. Transfer files to target hosts via SSH
4. Start services
5. Validate health

**Flags:**
- `--manifest string` - Cluster manifest file
- `--components string` - Provision only specific components (infrastructure, applications, interfaces)
...
frameworks cluster provision --components infrastructure

# Provision only applications
frameworks cluster provision --components applications
...
During provisioning, the CLI automatically bootstraps the system tenant and cluster:

1. **Service Token Required:** Set `SERVICE_TOKEN` environment variable with a valid service token for Quartermaster authentication.

2. **System Tenant Creation:** If the "FrameWorks" system tenant doesn't exist, it's automatically created with deployment tier "production".

3. **Cluster Registration:** A cluster record is created in Quartermaster with:
   - **Cluster ID:** Auto-generated as `{manifest.Type}-{manifest.Profile}` (e.g., `central-production`, `regional-staging`)
   - **Cluster Name:** Derived from manifest type
   - **Cluster Type:** From manifest (central, regional, edge)
   - **Base URL:** From manifest ingress configuration

4. **Infrastructure Bootstrap Token:** An infrastructure bootstrap token is created with:
   - Kind: `infrastructure_node`
   - TTL: 720 hours (30 days)
   - Single use (usage limit = 1)

**Important:** The auto-generated cluster ID format (`{type}-{profile}`) is needed when using:
...
frameworks cluster init --manifest production.yaml
```
...
**Flags:**
- `--manifest string` - Cluster manifest file
- `--skip-validation` - Skip health check after restart
- `--force` - Force stop (kill) if graceful shutdown fails
...
frameworks cluster restart bridge --skip-validation

# Force restart
frameworks cluster restart mistserver --force
```
...
Backup cluster components to local or remote storage.
...
**Output:**
- Files stored in `<output-dir>/<date>/`
- Timestamped filenames: `postgres-20250117-143000.sql`

**Flags:**
- `--manifest string` - Cluster manifest file
- `--output string` - Output directory (default: "./backups")
- `--skip-upload` - Skip remote storage upload (default: true)
...
- 2181 (Zookeeper)
- 9000 (ClickHouse)
- 18000-18099 (Application services)
- 80 (Nginx)
...
- `--kind` - Token kind: `edge_node` or `service` (default: `edge_node`)
- `--ttl` - Time-to-live (e.g., `24h`, `7d`) (default: `24h`)
- `--tenant-id` - Tenant ID (use `admin tenants list` to find)
- `--cluster-id` - Cluster ID (use `admin clusters list` to find)
- `--expected-ip` - Expected client IP for validation (optional)
- `--usage-limit` - Maximum uses, 0 = single use (default: `0`)
...
**Note:** Cluster IDs are auto-generated during provisioning with format `{type}-{profile}`.
...
# Filter by cluster
frameworks admin nodes list --cluster-id central-production
...
- `--type` - Filter by node type (`edge`, `regional`, `central`)
...
Edge node lifecycle operations. Most commands support `--ssh user@host` for remote execution.
...
Generate edge node configuration files (`.edge.env`, `docker-compose.edge.yml`, `Caddyfile`).
...
Start the edge stack and verify HTTPS readiness. The bootstrap token must be set in the `.edge.env` file as `EDGE_ENROLLMENT_TOKEN`.

```bash
# Enroll edge node (token read from .edge.env)
frameworks edge enroll
...
**Flags:**
- `--dir` - Directory containing `docker-compose.edge.yml` and `.edge.env`
...
Generate docker-compose configuration for central services.
...
Creates `~/.frameworks/config.yaml` with a "local" context pointing to `localhost` endpoints.
...
```bash
frameworks context set-url gateway http://api.example.com:18000
