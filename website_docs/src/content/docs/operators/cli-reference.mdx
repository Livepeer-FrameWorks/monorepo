...
For platform contexts, per-service non-local URLs saved with `frameworks context set-url` still override manifest-derived targets, but they must declare transport explicitly: use `--tls` for TLS endpoints or `--allow-insecure` for plaintext gRPC. Localhost defaults, manifest SSH tunnels, and mesh-derived addresses keep their established transport behavior. Localhost defaults are ignored in `ssh` and `mesh` modes so a newly created platform context does not accidentally dial the operator laptop for core services. The self-hosted setup wizard asks for the same transport choice when it captures owner Quartermaster/Foghorn endpoints.
...
mode and, unless `--version` is supplied, install from the cluster's release
...
override; owner contexts set targets against releases already present in the
catalog. Release rows must
