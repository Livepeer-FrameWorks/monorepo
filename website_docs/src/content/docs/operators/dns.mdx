...
**Navigator (`api_dns`)** provides automated DNS management for FrameWorks. It syncs DNS records based on Quartermaster node inventory (nodes with external IPs):
- **Single node** → A record (direct IP)
- **Multiple nodes** → Cloudflare load balancer pool with geo-routing

Navigator also handles ACME certificate issuance via DNS-01 validation (platform-managed subdomains only).
</Aside>
