...
- **DNS layer (Cloudflare + Bunny)**: Infrastructure-level geo-routing to nearest region or media cluster
- **Application layer (Foghorn)**: Capacity-aware routing to optimal edge node

> **Note**: Cloudflare remains the root/API/web/admin DNS provider. Bunny DNS handles delegated media cluster zones so edge growth does not consume Cloudflare Load Balancing endpoints.
