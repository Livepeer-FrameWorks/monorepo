...
| `edge-egress.example.com` | Edge pool domain | Edge nodes         |
| `logbook.example.com`     | Docs site        | Logbook            |
| `steward.example.com`     | Forms API        | Steward            |

Navigator only manages subdomains for service types that exist in Quartermaster inventory. If you want separate `edge-ingest` or `foghorn` domains, register nodes with those types; otherwise use the `edge-egress` pool.
...
| `edge-egress.{slug}.{base}` | Edge pool domain (viewer delivery)            |
...
```
tenant1.example.com         → CNAME → foghorn.example.com (or edge-egress.example.com)
