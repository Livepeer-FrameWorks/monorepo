...
- **Cross-cluster artifacts**: VOD clips on remote clusters are accessed via `PrepareArtifact` (presigned S3 URLs). DVR archive replay uses the GraphQL chapter API, which routes through Commodore to the DVR origin Foghorn and returns bounded chapter manifests.
