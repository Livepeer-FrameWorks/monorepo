...
FrameWorks supports multiple clusters. A **cluster** is a Foghorn instance (or HA pair) plus its edge nodes — strictly the media plane. Clusters share the central Control / API Plane (Commodore, Quartermaster, Purser). Event collection is regional, and Periscope Metering follows the logical ClickHouse deployment: one central source in today's unsharded topology, then one local worker and ClickHouse source per wider region after sharding. Purser remains the central financial authority.
