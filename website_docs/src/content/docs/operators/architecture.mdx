...
**Livepeer integration**: The Livepeer Gateway is provisioned via `frameworks livepeer` CLI commands and discovered via Quartermaster service discovery. MistServer sends segments to the gateway automatically when Livepeer transcoding is configured for a stream.
