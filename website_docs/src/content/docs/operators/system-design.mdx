...
Core design patterns, data flows, and security models.
...
Most media/runtime analytics originate as immutable events from the Edge (Media Plane), flowing
through Kafka into ClickHouse for real-time visibility and historical queries. Bridge and other
control-plane services also emit service events such as API usage batches.
...
| ------------------------------ | --------------------- | ------------------------------- |
| GraphQL complexity             | 1000                  | `GRAPHQL_COMPLEXITY_LIMIT`      |
...
| Cluster max concurrent streams | Cluster metadata      | Quartermaster database          |
| Cluster max concurrent viewers | Cluster metadata      | Quartermaster database          |

Pricing tiers include stream/viewer allowance fields, but runtime enforcement for paid-tier
stream/viewer limits is not currently active.
...
| `federation_event`      | Foghorn  | Cross-cluster peering and origin-pull events  | `federation_events`                                        |
...
- **Management**: Created and managed via the GraphQL API (Commodore). Tokens have customizable permissions and expiration dates.
