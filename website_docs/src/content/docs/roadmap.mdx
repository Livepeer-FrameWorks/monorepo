...
**Builds on:** edge clusters, virtual private routing slices, and the Edge ISO/cloud images below.
...
**Outcome:** run one policy model across FrameWorks-operated SaaS, customer edge capacity, and provider-authorized federation peers so residency requirements are enforced by the platform.
