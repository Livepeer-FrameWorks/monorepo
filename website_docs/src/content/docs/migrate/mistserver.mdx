...
| You need a customer-operated Foghorn, federation identity, or whole control plane                                             | Outside the supported FrameWorks edge product              | [Understand the product boundary](#customer-operated-control-plane-is-out-of-scope)                                                             |
...
## Customer-operated control plane is out of scope

The supported self-hosted product stops at MistServer, Helmsman, Caddy, and the customer edge host. FrameWorks operates Foghorn, federation identity, Livepeer, Chandler, object storage, DNS/TLS, analytics stores, billing, and the control plane.

Running Foghorn on customer infrastructure is not an advanced edge-install option. It requires a separate federation trust root and compatible deployments of the coupled media and durable services. The repository contains operator tooling for FrameWorks' own environments, but that tooling is not a supported customer whole-stack distribution or runbook.

If that provider boundary does not meet your requirements, remain on raw MistServer or evaluate a separately scoped platform rather than treating edge enrollment as authority to deploy the FrameWorks control plane.
