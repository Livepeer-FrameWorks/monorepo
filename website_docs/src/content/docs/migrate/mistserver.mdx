...
We co-maintain MistServer. Start with the table below; for some workloads raw MistServer remains the better fit.
...
| You want your own Foghorn, federation identity, and control plane                                                             | Full self-host of the entire stack                         | [Self-host the whole stack](#run-your-own-foghorn-self-host-the-whole-stack)                                                                    |
...
<Aside type="note">
  If your workload matches the first row, stay on raw MistServer. It is the better fit when you need
  direct config control, local scripts, or the full protocol and process surface.
</Aside>

## TL;DR

- **What stays:** RTMP, E-RTMP, SRT, and WHIP ingest; HLS, LL-HLS, DASH, WebRTC/WHEP, MP4, WebM/MKV, and WebSocket playback; MistServer as the media edge.
- **What changes:** Mist JSON config and HTTP API calls become GraphQL mutations; Mist triggers become signed webhooks, GraphQL subscriptions, or platform-managed webhook policies; per-node Prometheus becomes FrameWorks telemetry; `pushStart` and automatic pushes become stream-scoped push targets.
- **What you gain:** provider-operated cross-cluster routing and federation, managed GraphQL, sprite-sheet seek previews, `?jwt=` playback auth across protocols, Skipper diagnostics, managed registration for Mist pull sources, wallet auth, MCP, x402, and a future marketplace path for operator-owned edge capacity.
...
A paid tenant gets one hostname namespace that routes across its own nodes, FrameWorks platform clusters, and marketplace clusters. This is most relevant for MistServer operators, who already run their own infrastructure.
...
mutation CreateMistPullStream {
  createStream(
    input: {
      ...
      ingestMode: PULL
      pullSource: { sourceUri: "dtsc://mist-origin.example.com:4200/main?pass=shared-secret" }
    }
  ) {
    ... on Stream {
      id
...
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| RTMP / E-RTMP push               | Push stream with the FrameWorks ingest URL and stream key                                                         |
...
MistServer triggers can POST to URLs or execute local programs. Some are blocking and can change behavior. FrameWorks replaces the common non-blocking event cases with its public events, delivered as signed [webhooks](/builders/webhooks) or over the `tenantEvents` subscription, plus per-stream telemetry subscriptions, and reserves blocking authorization for playback policies.

| MistServer trigger                                                                      | FrameWorks path                                                                                  |
| --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `STREAM_LOAD` / stream lifecycle                                                        | `stream.connected`, `stream.live`, `stream.idle` events; `liveStreamEvents(streamId)` for detail |
...
| `RECORDING_END` / DVR lifecycle                                                         | `recording.ready`, `recording.failed`, `clip.ready`, `clip.failed` events                        |
| Push completion or failure                                                              | `multistream.status_changed` event                                                               |
...
6. **Move traffic gradually.** Switch player embeds, DNS, or encoder presets only after one full traffic cycle. Keep raw Mist available as rollback until the edge has run a full traffic cycle without incidents.
...
## Run your own Foghorn: self-host the whole stack

With a hybrid edge, you run MistServer, Helmsman, Caddy, and the edge host. FrameWorks operates Foghorn, federation identity, Livepeer, Chandler, object storage, DNS/TLS, analytics stores, billing, and the control plane.

Running Foghorn on your own infrastructure is not an edge-install option. Foghorn depends on a federation trust root and on the coupled media and durable services, so running it yourself means self-hosting the entire stack: control plane, Foghorn, edges, Livepeer, Chandler, databases, analytics, and interfaces. FrameWorks is public domain (Unlicense), and full self-hosting is supported: the `frameworks` CLI deploys and upgrades the stack from a GitOps repository, as described in the [operator guides](/operators/deployment).

A self-hosted platform is independent. Attaching your own Foghorn or control plane to the FrameWorks network, or mixing its trust root with the FrameWorks platform's, is not supported.
...
4. **Repoint encoders.** RTMP, E-RTMP, SRT, and WHIP remain the normal managed ingest paths.
5. **Replace trigger scripts.** Point trigger-driven HTTP handlers at a [webhook endpoint](/builders/webhooks) for the public events, or use subscriptions. For Mist `USER_NEW` authorization scripts, use `setPlaybackPolicy` with `type: WEBHOOK` so FrameWorks calls your authorization endpoint on viewer connect.
...
- **Hybrid-edge production cutovers need operator validation.** Test DNS, TLS,
