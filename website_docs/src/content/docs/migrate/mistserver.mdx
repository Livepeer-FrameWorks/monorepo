...
FrameWorks does not make pull inputs new; MistServer already does this by setting the stream source. What FrameWorks adds is a managed record for that source URI, encrypted storage, placement rules, and an `allow_private_pull_sources` gate for self-hosted edges that may reach RFC1918 or multicast sources. See [Pull-input streams](/builders/pull-streams).
...
Hybrid edges are experimental today. They are less flexible than raw MistServer because Helmsman syncs managed config from FrameWorks, but that boundary can be loosened for real operator needs through account-level overrides or custom config extension points. If you need SDI workflows, UDP hole punching, custom trigger behavior, or direct Mist overrides, talk to us before treating the managed surface as a hard no.
...
Sovereign does not mean every subsystem is swappable today. The current stack is opinionated: crypto rails, wallet/x402 surfaces, Purser billing, and the Livepeer integration are part of the platform shape even when you run it yourself. If you want FrameWorks without crypto, want to bring your own invoicing/billing system, or want to disable whole product areas instead of leaving them unused, talk to us first. Those are valid sovereign requirements, but the knobs are not all first-class yet.
...
- Future operator marketplace: once public cluster publishing is ready, operators with spare compute or bandwidth should be able to sell that capacity instead of only using it for their own traffic.

## Beta and tricky bits

- **Hybrid edge deployment is experimental.** It is useful for testing and early operators, but it is not the same maturity level as running raw MistServer yourself. We can extend it around concrete customer needs; do not assume the current Helmsman config boundary is final.
...
- **Sovereign installs are still opinionated.** Purser does not yet act as a generic adapter for external billing/invoicing, and crypto-oriented surfaces such as wallet auth and x402 are not fully removable product modules today.
- **The operator marketplace is still in progress.** You do not lose anything today; the upside is future public cluster publishing, where operators can sell excess compute and bandwidth through FrameWorks instead of keeping every edge private.
- **Billing is in beta.** Rates and allowances may change as the platform matures.
