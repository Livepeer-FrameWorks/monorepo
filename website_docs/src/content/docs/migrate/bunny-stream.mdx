...
- **What's not yet at parity:** DRM (FairPlay/Widevine/PlayReady in development) · automatic chapter generation and transcription/caption auto-generation · per-video heatmaps and retention curves · per-region replication knob · iframe-embed-as-a-product · first-class Python/PHP/C#/Java/Go SDKs · outbound HTTP webhooks · VOD URL-pull import · watermarking as a managed feature.
...
<Aside type="note">
  FrameWorks is in active beta. Billing rates, hybrid edge deployment, the cluster marketplace, and
  DRM are all flagged below — read the [beta and tricky bits](#beta-and-tricky-bits) section before
  you cut over production traffic.
</Aside>
...
Pull-input streams use `createStream(input: { ingestMode: PULL, pullSource: { sourceUri } })` — see [Pull-input streams](/builders/pull-streams). Self-hosted edges can pull from RFC1918 / multicast sources via an explicit `allow_private_pull_sources` cluster flag.
...
<Aside type="caution">
  ...
  tracked in the outbound-webhooks RFC.
</Aside>
...
- **MediaCage Enterprise Multi-DRM** → DRM (FairPlay / Widevine / PlayReady) is in development with the [MistServer](https://mistserver.org/) team; we co-maintain the upstream engine. Wait for DRM GA or stay on Bunny for that subset.
...
- **Pull-input streams** — point FrameWorks at an RTSP camera, an SRT/RIST contribution feed, an HLS or MPEG-TS source, or another Mist origin (DTSC). Self-hosted edges can also pull from private (RFC1918) or multicast sources via an explicit cluster flag. See [Pull-input streams](/builders/pull-streams). On the [roadmap](/roadmap): an **AV Device Discovery** drop-in app for ONVIF cameras, VISCA PTZ, NDI sources, USB webcams, and HDMI capture.
...
- **DRM** (FairPlay / Widevine / PlayReady) — in development with the [MistServer](https://mistserver.org/) team.
- **Outbound HTTP webhooks** — GraphQL subscriptions today; the outbound-webhook surface is tracked in an RFC.
...
- **Per-video heatmaps and retention curves** — QoE Analytics RFC.
...
- **First-class Python, PHP, C#, Java, Go SDKs** — internal Go clients exist; use GraphQL codegen against the schema until first-party SDKs ship.
...
- **Hybrid edge deployment** is experimental — fine for testing, not for production-critical traffic.
