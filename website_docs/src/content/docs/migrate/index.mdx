...
description: "FrameWorks is an alternative to video APIs like Mux, Bunny Stream, and Livepeer Studio, with managed and hybrid-edge deployment options."
sidebar:
  order: 1
---
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Available now — VOD/CDN swap, plus RTMP/SRT/WHIP live ingest, which Bunny Stream doesn't offer today."
  />
  <LinkCard
    title="Mux"
    href="/migrate/mux"
...
2. **Repoint your encoder.** RTMP, E-RTMP, WHIP, or SRT — the URL changes; the protocols stay. RTMPS encoders move to RTMP, or to SRT or WHIP for an encrypted link.
...
- **Webhooks and subscriptions share one event catalog.** The public events (streams, clips,
  recordings, uploads, multistream targets, billing, account) arrive as Standard Webhooks-signed
  [webhooks](/builders/webhooks) or live over the `tenantEvents` subscription, with the same
  payloads. High-volume telemetry such as viewer connections stays subscription-only.
- **Wallet auth + x402 for agents.** Autonomous callers don't need an email signup. Sign with an EVM wallet (EIP-191) to get a tenant, then top it up with a gasless USDC payment (EIP-3009); x402 funds an authenticated tenant and is never a login credential. See [Agent Access](/agents/overview).
- **Managed hybrid edges.** You can run MistServer and Helmsman edge nodes on your hardware while FrameWorks operates Foghorn, federation, Livepeer, Chandler, object storage, DNS/TLS, analytics, billing, and the control plane.
...
<Aside type="caution" title="Before you plan a production cutover">
  Items still in motion across the platform: published billing rates, hybrid-edge runbooks, the
  vetted public marketplace program, DRM, and provider-specific migration gaps. Each per-source
