...
| `set_node_mode`             | Set node mode (production/standby/maintenance) | Auth              |
| `get_node_health`           | Node health and metrics summary                | Auth              |
| `create_private_cluster`    | Create private cluster (returns token)         | Auth              |

<Aside type="tip" title="CLI Alternative">
  For operators who prefer the CLI over MCP, `frameworks edge deploy` combines
  `create_private_cluster` and `create_enrollment_token` into a single command that also provisions
  the edge node. See the [CLI Reference](/operators/cli-reference#edge-deploy) and [Connecting Your
...
- `mode`: Set to `docs` for read-only mode (default `full`)

Returns a structured response with `answer`, `confidence` (`verified`, `sourced`, `best_guess`, or `unknown`), `sources`, and `tools_used`. This runs the full Skipper pipeline — knowledge retrieval, web search, query rewriting, and multi-step reasoning — so agents get the same quality as the dashboard chat.
...
Use `ask_consultant` for knowledge lookups with confidence tagging, or the diagnostic tools below for stream-level troubleshooting.
