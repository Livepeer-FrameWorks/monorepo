...
| `topup_balance`             | Request USDC deposit address                   | Auth           |
| `check_topup`               | Check crypto payment status                    | Auth           |
| `create_stream`             | Create a new stream                            | Auth + Balance |
| `update_stream`             | Update stream settings                         | Auth + Balance |
...
<Aside type="tip" title="CLI Alternative">
  ...
  the edge node. See the [CLI Reference](/operators/cli-reference#frameworks-edge-deploy) and
  [Connecting Your Edge](/hybrid/connecting) for details.
</Aside>
...
- `asset`: USDC. The Gateway schema currently accepts ETH and LPT, but the prepaid top-up backend rejects them until conversion-backed top-ups are implemented.
