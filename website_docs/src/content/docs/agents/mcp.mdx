...
| `set_retention_policy`           | Set tenant media-retention defaults               | Auth           |
| `set_stream_retention_overrides` | Set DVR/clip retention overrides for a stream     | Auth           |
| `update_asset_retention`         | Override retention for DVR, clip, or VOD          | Auth + Balance |
| `reset_asset_retention`          | Reset asset retention to the cascade default      | Auth + Confirm |
