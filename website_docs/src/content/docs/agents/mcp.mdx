...
Actions the agent can perform. 30 tools across 12 categories.
...
| `search_knowledge`          | Semantic search over Skipper's knowledge base | Auth              |
| `search_web`                | Live web search via configured provider       | Auth              |
...
- `query` (required): Search query to run against the knowledge base
- `limit`: Maximum results to return (default 5)
- `tenant_scope`: Scope to search — `tenant`, `global`, or `all` (default `all`)

**search_web**

- `query` (required): Search query to run against the web
- `limit`: Maximum results to return (default 5)
...
Use `search_knowledge` and `search_web` for targeted lookups, or the diagnostic tools below for stream-level troubleshooting.
...
5. AI calls `search_knowledge("rebuffering troubleshooting")` for relevant documentation
