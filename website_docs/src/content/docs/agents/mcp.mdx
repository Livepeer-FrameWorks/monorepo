...
Public resources (no auth required): `account://status`, `billing://pricing`, `knowledge://sources`.
...
Actions the agent can perform. 27 tools across 10 categories.
...
| `introspect_schema`         | Explore the GraphQL schema                | None              |
| `generate_query`            | Generate a GraphQL query from templates   | None              |

Public tools (no auth required): `get_payment_options`, `submit_payment`, `resolve_playback_endpoint`, `introspect_schema`, `generate_query`.
