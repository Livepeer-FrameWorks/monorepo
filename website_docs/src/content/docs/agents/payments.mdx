...
<Aside type="note">
  x402 only works with USDC (EIP-3009 required). Prepaid crypto top-ups currently accept USDC only.
</Aside>

## Crypto Deposits

For non-x402 USDC top-ups, use the deposit flow:

1. Call `topup_balance` with `amount_cents` and `asset: "USDC"`.
...
ETH and LPT prepaid top-ups are not currently accepted by the prepaid top-up backend.
