...
4. Server verifies the signature, creates a tenant if new, sets auth cookies, and returns user metadata.
...
Alternatively, use the GraphQL `walletLogin` mutation to exchange the same address/message/signature fields for a JWT, then use `Authorization: Bearer <jwt>` on subsequent requests. The REST `POST /auth/wallet-login` endpoint is cookie-oriented.
