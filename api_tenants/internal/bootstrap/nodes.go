...
func ReconcileNodes(ctx context.Context, exec DBTX, nodes []Node) (Result, error) {
	...
	for _, n := range nodes {
		...
		action, err := upsertNode(ctx, exec, n)
		if err != nil {
...
func upsertNode(ctx context.Context, exec DBTX, n Node) (string, error) {
	const probeSQL = `
		SELECT
	...
	case errors.Is(err, sql.ErrNoRows):
		...
		if _, insertErr := exec.ExecContext(ctx, insertSQL,
			...
			n.ExternalIP, n.WireGuard.IP, n.WireGuard.PublicKey, n.WireGuard.Port,
		); insertErr != nil {
	...
	desiredName := n.ID
	if curName == desiredName && curType == n.Type && curWGPort == n.WireGuard.Port {
		return "noop", nil
	}
	...
	const updateSQL = `
		...
		SET node_name = $2,
		    ...
		    updated_at = NOW()
		WHERE node_id = $1`
	if _, err := exec.ExecContext(ctx, updateSQL, n.ID, desiredName, n.Type, n.WireGuard.Port); err != nil {
