...
type ListTenantEffectiveAccessRow struct {
	...
	MediaConsentRevision     int64          `db:"media_consent_revision" json:"media_consent_revision"`
	...
	MediaAllowExternalSource bool           `db:"media_allow_external_source" json:"media_allow_external_source"`
...
func (q *Queries) ListTenantEffectiveAccess(ctx context.Context, tenantID string) ([]ListTenantEffectiveAccessRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&i.MediaConsentRevision,
			&i.MediaAllowIngest,
			&i.MediaAllowServe,
			&i.MediaAllowExternalSource,
			&i.RegionID,
		); err != nil {
			return nil, err
		}
