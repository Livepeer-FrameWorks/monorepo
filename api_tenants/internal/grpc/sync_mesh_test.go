...
func TestSyncMeshUpdatesHeartbeatWithoutKeyOrPort(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT s\.type, n\.wireguard_ip::text\s+FROM quartermaster\.services s\s+JOIN quartermaster\.service_instances si ON si\.service_id = s\.service_id\s+JOIN quartermaster\.infrastructure_nodes n ON n\.node_id = si\.node_id\s+WHERE si\.status IN \('running', 'active'\)\s+AND n\.wireguard_ip IS NOT NULL\s+AND n\.status = 'active'\s+AND n\.cluster_id = \$1\s+AND s\.type IS NOT NULL AND s\.type <> ''`).
		WithArgs("cluster-1").
...
func TestSyncMeshServiceEndpointsKeyedByType(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT wireguard_ip::text, external_ip::text, internal_ip::text, cluster_id`).
		WithArgs("node-1").
		WillReturnRows(sqlmock.NewRows([]string{"wireguard_ip", "external_ip", "internal_ip", "cluster_id"}).AddRow("10.200.0.5", "1.2.3.4", "10.0.0.5", "cluster-1"))

	mock.ExpectExec(`UPDATE quartermaster\.infrastructure_nodes`).
		WithArgs("pub-key-1", int32(51820), "node-1").
		WillReturnResult(sqlmock.NewResult(0, 1))
