	...
	return []*periscopepb.StorageEvent{
		{
			...
			AssetType: "clip",
			SizeBytes: 25000000,
			S3Url:     stringPtr("s3://frameworks-demo/clips/demo_live_stream_001/a1b2c3d4e5f6789012345678901234ab.mp4"),
			LocalPath: stringPtr("/var/lib/mistserver/recordings/clips/demo_live_stream_001/a1b2c3d4e5f6789012345678901234ab.mp4"),
			NodeId:    "node_demo_us_west_01",
		},
		{
			Id:             "storage_demo_002",
	...
	events := []*periscopepb.ClipEvent{
		{
			...
			FilePath:     stringPtr("/var/lib/mistserver/recordings/clips/demo_live_stream_001/a1b2c3d4e5f6789012345678901234ab.mp4"),
			S3Url:        stringPtr("s3://frameworks-demo/clips/demo_live_stream_001/a1b2c3d4e5f6789012345678901234ab.mp4"),
...
func GenerateArtifactState(requestID string) *periscopepb.ArtifactState {
	...
	return &periscopepb.ArtifactState{
		...
		FilePath:        stringPtr("/var/lib/mistserver/recordings/clips/demo_live_stream_001/" + requestID + ".mp4"),
		S3Url:           stringPtr("s3://frameworks-demo/clips/demo_live_stream_001/" + requestID + ".mp4"),
		SizeBytes:       uint64Ptr(15000000),
		UpdatedAt:       timestamppb.New(now.Add(-1 * time.Hour)),
	}
}
...
func GenerateStorageEventsConnection(internalName *string) *model.StorageEventsConnection {
	...
	events := []*periscopepb.StorageEvent{
		...
		},
		{
			...
			LocalPath:      stringPtr("/var/lib/mistserver/recordings/dvr/demo_live_stream_001/fedcba98765432109876543210fedcba/fedcba98765432109876543210fedcba.m3u8"),
			NodeId:         "node_demo_us_west_01",
...
func GenerateFederationEventsConnection() *model.FederationEventsConnection {
	...
	events := []*periscopepb.FederationEvent{
		...
		{Timestamp: timestamppb.New(now.Add(-20 * time.Minute)), EventType: "federation_query", LocalCluster: "us-east-edge", RemoteCluster: "", StreamName: stringPtr("demo_live_stream_004"), QueriedClusters: uint32Ptr(2), RespondingClusters: uint32Ptr(0), TotalCandidates: uint32Ptr(2), FailureReason: stringPtr("no peers returned stream"), LatencyMs: float32Ptr(80.0), Role: "leader"},
		...
		{Timestamp: timestamppb.New(now.Add(-50 * time.Minute)), EventType: "peer_disconnected", LocalCluster: "central-primary", RemoteCluster: "apac-edge", PeerCluster: stringPtr("apac-edge"), FailureReason: stringPtr("heartbeat timeout"), Role: "leader"},
...
func GenerateNetworkStatus() *model.NetworkStatus {
	platformDesc := "Platform cluster: all control, data, and foghorn services"
	mediaDesc := "Media cluster: edge nodes, stream routing, viewer delivery"
	...
	return &model.NetworkStatus{
		Clusters: []*model.NetworkClusterStatus{
			{ClusterID: "central-primary", Name: "Central Platform", Region: "US-Central", Latitude: 41.8781, Longitude: -87.6298, NodeCount: 2, HealthyNodeCount: 2, PeerCount: 2, Status: "healthy", ClusterType: infra.ClusterTypeCentral, ShortDescription: &platformDesc, CurrentStreams: 0, CurrentViewers: 0, EgressMbps: 0, EgressCapacityMbps: 0, IngressMbps: 0, Services: []string{"bridge", "commodore", "decklog", "foghorn", "periscope-ingest", "purser", "signalman", "skipper"}},
			{ClusterID: "demo-media", Name: "Demo Media Cluster", Region: "US-Central", Latitude: 41.9000, Longitude: -87.6500, NodeCount: 3, HealthyNodeCount: 3, PeerCount: 0, Status: "healthy", ClusterType: infra.ClusterTypeEdge, ShortDescription: &mediaDesc, CurrentStreams: 12, CurrentViewers: 340, EgressMbps: 1200, EgressCapacityMbps: 10000, IngressMbps: 300, Services: []string{"helmsman"}},
			{ClusterID: "us-east-edge", Name: "US East Edge", Region: "US-East", Latitude: 40.7128, Longitude: -74.0060, NodeCount: 3, HealthyNodeCount: 3, PeerCount: 1, Status: "healthy", ClusterType: infra.ClusterTypeEdge, ShortDescription: &usEastDesc, CurrentStreams: 5, CurrentViewers: 180, EgressMbps: 600, EgressCapacityMbps: 5000, IngressMbps: 150, Services: []string{"foghorn", "helmsman"}},
			{ClusterID: "apac-edge", Name: "APAC Edge", Region: "AP-Northeast", Latitude: 35.6762, Longitude: 139.6503, NodeCount: 2, HealthyNodeCount: 2, PeerCount: 1, Status: "healthy", ClusterType: infra.ClusterTypeEdge, ShortDescription: &apacDesc, CurrentStreams: 3, CurrentViewers: 95, EgressMbps: 280, EgressCapacityMbps: 3000, IngressMbps: 80, Services: []string{"foghorn", "helmsman"}},
		},
		...
		ServiceInstances: []*model.NetworkServiceInstance{
			...
			{InstanceID: "bridge-central-01", ServiceID: "bridge", ClusterID: "central-primary", NodeID: stringPtr("central-node-1"), Status: "running", HealthStatus: "healthy"},
			{InstanceID: "commodore-central-01", ServiceID: "commodore", ClusterID: "central-primary", NodeID: stringPtr("central-node-1"), Status: "running", HealthStatus: "healthy"},
			{InstanceID: "purser-central-01", ServiceID: "purser", ClusterID: "central-primary", NodeID: stringPtr("central-node-2"), Status: "running", HealthStatus: "healthy"},
			{InstanceID: "skipper-central-01", ServiceID: "skipper", ClusterID: "central-primary", NodeID: stringPtr("central-node-2"), Status: "running", HealthStatus: "healthy"},
			{InstanceID: "signalman-central-01", ServiceID: "signalman", ClusterID: "central-primary", NodeID: stringPtr("central-node-1"), Status: "running", HealthStatus: "healthy"},
			{InstanceID: "decklog-central-01", ServiceID: "decklog", ClusterID: "central-primary", NodeID: stringPtr("central-node-2"), Status: "running", HealthStatus: "healthy"},
			{InstanceID: "periscope-central-01", ServiceID: "periscope-ingest", ClusterID: "central-primary", NodeID: stringPtr("central-node-2"), Status: "running", HealthStatus: "healthy"},
			// Foghorn HA pair on platform cluster (serves demo-media via cluster_assignments)
			{InstanceID: "foghorn-1", ServiceID: "foghorn", ClusterID: "central-primary", NodeID: stringPtr("central-node-1"), Status: "running", HealthStatus: "healthy"},
			{InstanceID: "foghorn-2", ServiceID: "foghorn", ClusterID: "central-primary", NodeID: stringPtr("central-node-2"), Status: "running", HealthStatus: "healthy"},
			// Edge services
			{InstanceID: "foghorn-useast-01", ServiceID: "foghorn", ClusterID: "us-east-edge", NodeID: stringPtr("useast-core-1"), Status: "running", HealthStatus: "healthy"},
