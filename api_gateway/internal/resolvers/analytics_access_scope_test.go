...
func TestAnalyticsAccessScopeContracts(t *testing.T) {
	...
	t.Run("network status splits public and owner views", func(t *testing.T) {
		...
		assertContains(t, src, "exposedNodeClusterIDs")
		assertContains(t, src, "exposedServiceInstanceClusterIDs")
		assertContains(t, src, "topologyServiceInstancesRaw")
		assertContains(t, src, "appendClusters(publicClustersResp.GetClusters(), true, true, true, false)")
		assertContains(t, src, "appendClusters(accessResp.GetClusters(), true, false, false, false)")
		assertContains(t, src, "appendClusters(ownedClustersResp.GetClusters(), true, false, true, true)")
		...
		assertContains(t, src, "if _, expose := exposedNodeClusterIDs[n.ClusterId]; expose")
