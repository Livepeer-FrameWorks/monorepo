...
func (a *Agent) sync() {
	...
	identityErr := a.validateManagedSelfIdentity(resp)
	if identityErr != nil {
		a.logger.WithError(identityErr).Error("Quartermaster returned conflicting mesh self identity")
		a.syncFailed()
		return
	}
...
func (a *Agent) writeServiceCertificate(serviceType, certPEM, keyPEM string) error {
	...
	dirMode := os.FileMode(0o750) | os.ModeSetgid
