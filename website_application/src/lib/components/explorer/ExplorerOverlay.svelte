...
interface Schema {
  ...
  const filteredTypes = $derived.by(() => {
    ...
    return schema.types
      .filter((type) => {
        ...
        // Search in fields
        if (type.fields?.some(f =>
          f.name?.toLowerCase().includes(search) ||
          f.description?.toLowerCase().includes(search)
        )) return true;

        // Search in input fields
        if (type.inputFields?.some(f =>
          f.name?.toLowerCase().includes(search) ||
          f.description?.toLowerCase().includes(search)
        )) return true;

        // Search in enum values
        if (type.enumValues?.some(e =>
  ...
  const totalTypesCount = $derived.by(() => {
    ...
    const builtins = new Set(["String", "Int", "Float", "Boolean", "ID", "Time", "DateTime", "JSON"]);
    return schema.types.filter(t =>
      t.name && !t.name.startsWith('__') && !builtins.has(t.name) &&
  ...
  function buildTypeUsageIndex(): Map<string, UsedByRef[]> {
    ...
    const scanFields = (fields: SchemaField[] | undefined, parentType: string, operationType?: 'Query' | 'Mutation' | 'Subscription') => {
      ...
      {#if view === 'templates'}
        ...
        <div class="px-4 py-2 border-b border-border/30">
          <div class="flex items-center gap-2">
            <button
              class="px-2 py-1 text-[11px] border border-border/50 transition-colors {libraryTab === 'curated' ? 'bg-primary text-primary-foreground border-primary' : 'text-muted-foreground hover:text-foreground hover:bg-muted/30'}"
              onclick={() => (libraryTab = 'curated')}
            >
              Curated
            </button>
            <button
              class="px-2 py-1 text-[11px] border border-border/50 transition-colors {libraryTab === 'all' ? 'bg-primary text-primary-foreground border-primary' : 'text-muted-foreground hover:text-foreground hover:bg-muted/30'}"
            ...
            >
              ...
              {#if libraryTab === "curated"}
                <div class="px-4 py-2 flex items-center justify-between text-[11px] text-muted-foreground border-t border-border/30">
                  <span>
        ...
        {#if queryHistory.length > 0}
          <div class="border-t border-border/30 mt-4">
            <div class="px-4 py-2 text-xs font-medium text-muted-foreground uppercase tracking-wider bg-muted/30 flex items-center gap-2">
              <Clock class="w-3 h-3" />
              Recent History
      ...
      {:else if view === 'schema'}
        {#if loading}
              ...
              {:else if schema}
                ...
                {#if navigationStack.length > 1}
                  <div class="px-3 py-2 border-t border-border/30 bg-muted/30 flex flex-wrap items-center gap-1 text-xs">
                    <button
                    ...
                    {#each navigationStack as nav, i (i)}
                      ...
                      {/if}
                      <button
                        class="px-1.5 py-0.5 hover:bg-muted rounded transition-colors font-mono {i === navigationStack.length - 1 ? 'text-primary font-medium' : 'text-muted-foreground'}"
                    ...
                    <div class="border-t border-border/30">
                      ...
                      <div class="px-4 py-3 bg-muted/20">
                        <div class="flex items-center gap-2 mb-1">
                          <span class="text-[10px] uppercase tracking-wide text-muted-foreground px-1.5 py-0.5 bg-muted border border-border/40">
                            {selectedType.kind}
                          </span>
                            ...
                            {#each selectedType.enumValues as enumVal (enumVal.name)}
                              <div class="text-xs pl-2 border-l-2 border-border/30">
                                ...
                                {#if enumVal.isDeprecated}
                                  <span class="text-warning text-[10px]">Deprecated{enumVal.deprecationReason ? `: ${enumVal.deprecationReason}` : ''}</span>
                                {/if}
                              </div>
                            {/each}
                          </div>
                              ...
                              <div class="text-xs pl-2 border-l-2 border-border/30">
                                ...
                                {#if typeField.isDeprecated}
                                  <div class="flex items-center gap-1 mt-0.5 text-warning">
                                    ...
                                    <span class="text-[10px]">Deprecated{typeField.deprecationReason ? `: ${typeField.deprecationReason}` : ''}</span>
                                  </div>
                                {/if}
                              </div>
                            {/each}
                          ...
                          <div class="flex flex-wrap gap-1">
                            {#each usages.slice(0, 10) as usage, i (`${usage.parentType}.${usage.fieldName}-${i}`)}
                              <button
                                ...
                                title={usage.operationType ? `${usage.operationType} (root type)` : `Navigate to ${usage.parentType}`}
                              >
                                <span class="text-muted-foreground">{usage.parentType}.</span><span class="text-foreground">{usage.fieldName}</span>
                              </button>
                            {/each}
                            {#if usages.length > 10}
                ...
                <div class="p-3 border-t border-border/30">
                  ...
                  {#if schemaSearch.trim()}
                    <div class="mt-2 text-[10px] text-muted-foreground">
                      Found: {filteredQueries.length} queries, {filteredMutations.length} mutations, {filteredSubscriptions.length} subscriptions, {filteredTypes.length} types
                    </div>
                ...
                {#if filteredQueries.length > 0}
                  <div class="border-t-2 border-border/50">
                    ...
                    >
                      <span>
                        Queries
                        <span class="text-muted-foreground ml-1">({filteredQueries.length}{schemaSearch.trim() && schema.queryType?.fields ? `/${schema.queryType.fields.length}` : ''})</span>
                      </span>
                      <ChevronDown class="w-4 h-4 transition-transform {collapsedSections.has('queries') ? '-rotate-90' : ''}" />
                    </button>
                ...
                {#if filteredMutations.length > 0}
                  <div class="border-t-2 border-border/50">
                    ...
                    >
                      <span>
                        Mutations
                        <span class="text-muted-foreground ml-1">({filteredMutations.length}{schemaSearch.trim() && schema.mutationType?.fields ? `/${schema.mutationType.fields.length}` : ''})</span>
                      </span>
                      <ChevronDown class="w-4 h-4 transition-transform {collapsedSections.has('mutations') ? '-rotate-90' : ''}" />
                    </button>
                ...
                {#if filteredSubscriptions.length > 0}
                  <div class="border-t-2 border-border/50">
                    ...
                    >
                      <span>
                        Subscriptions
                        <span class="text-muted-foreground ml-1">({filteredSubscriptions.length}{schemaSearch.trim() && schema.subscriptionType?.fields ? `/${schema.subscriptionType.fields.length}` : ''})</span>
                      </span>
                      <ChevronDown class="w-4 h-4 transition-transform {collapsedSections.has('subscriptions') ? '-rotate-90' : ''}" />
                    </button>
                ...
                {#if filteredTypes.length > 0}
                  <div class="border-t-2 border-border/50">
                    ...
                    >
                      <span>
                        Types
                        <span class="text-muted-foreground ml-1">({filteredTypes.length}{schemaSearch.trim() ? `/${totalTypesCount}` : ''})</span>
                      </span>
                      <ChevronDown class="w-4 h-4 transition-transform {collapsedSections.has('types') ? '-rotate-90' : ''}" />
                    </button>
                      ...
                      {#each filteredTypes as type (type.name)}
                        <div class="group">
                          ...
                          >
                            <div class="flex items-center justify-between gap-2">
                              <div class="flex items-center gap-2 min-w-0">
                                <span class="font-mono text-primary hover:underline truncate">{type.name}</span>
                              </div>
                              <div class="flex items-center gap-1 flex-shrink-0">
                                <span class="text-[10px] uppercase tracking-wide text-muted-foreground">
                                  {type.kind}
                            ...
                            {#if type.description}
                              <p class="text-[10px] text-muted-foreground mt-0.5 truncate">{type.description}</p>
                            {/if}
                          </button>
                        </div>
                      {/each}
...
{#if sidebarMode && view}
  <div class="w-80 bg-card border-r border-border h-full overflow-hidden flex flex-col shrink-0">
    <div class="p-3 border-b border-border flex items-center justify-between flex-shrink-0">
      ...
      <button class="p-1 text-muted-foreground hover:text-foreground transition-colors" onclick={onClose}>
        <X class="w-4 h-4" />
      </button>
    </div>
