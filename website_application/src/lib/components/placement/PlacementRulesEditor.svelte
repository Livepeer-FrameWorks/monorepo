...
>
  ...
  <div class="flex flex-wrap gap-2">
    <Button variant={rules ? "outline" : "secondary"} onclick={() => onchange(null)}>
      {scope.kind === "STREAM" ? "Use account rules" : "Use system defaults"}
    </Button>
    <Button variant={rules ? "secondary" : "outline"} onclick={() => change(() => {})}
      >Customize {scope.kind === "STREAM" ? "this stream" : "account rules"}</Button
    >
  </div>
  {#if !rules}
    <p class="text-sm text-muted-foreground">
      No local override. Clearing previously saved rules is a reviewed change because it may widen
      access.
    </p>
  {/if}
  <div class="space-y-2">
    <label class="text-sm block"
      ...
      <select
        ...
        bind:value={preset}
        onchange={() => (replacePending = false)}
      >
        <option value="">Choose a preset…</option>
        {#each supported as id (id)}<option value={id}>{presetLabels[id]}</option>{/each}
      </select>
    </label>
    <Button
      size="sm"
      variant="outline"
      disabled={!preset}
      onclick={() => (rules ? (replacePending = true) : applyPreset())}>Use preset</Button
    >
    {#if replacePending}
      <div class="border-l-2 border-warning pl-3 text-sm space-y-2">
        <p>
          Replace this tab’s entire draft, including its local restrictions? Account restrictions
          inherited by a stream remain enforced.
        </p>
        <Button size="sm" onclick={applyPreset}>Replace this tab’s draft</Button>
  ...
  {#if rules}
    <section class="space-y-3 border-t border-border pt-4" aria-label="Hard restrictions">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <Button
          ...
          onclick={() =>
            change((draft) =>
              draft.constraints.deny.push({ classes: ["PLATFORM_OFFICIAL"], charging: ["RATED"] })
            )}>Exclude rated official</Button
        >
        <Button
          size="sm"
          variant="outline"
          onclick={() => change((draft) => draft.constraints.deny.push({}))}>Add custom deny</Button
        >
      </div>
      <p class="text-xs text-muted-foreground">
        An empty custom deny matches every destination. Choose its matching fields before reviewing.
      </p>
    </section>
              ...
              {#if group.order === "PRICE"}
                ...
                <label class="text-sm block"
                  >Comparable unit<Input
                    ...
                    onchange={(event) => editGroup(index, { priceUnit: event.currentTarget.value })}
                ...
                <div class="pt-3 space-y-3">
                  ...
                  <label class="text-sm block"
                    ...
                    >
                      ...
                      >
                      <option value="CAPACITY_OR_GEO_HOLE" disabled={!features.geographicSpillover}
