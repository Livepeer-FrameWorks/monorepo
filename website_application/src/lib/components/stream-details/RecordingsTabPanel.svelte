    ...
    <div class="slab-body--flush">
      {#each recordings as recording (recording.dvrHash)}
        ...
        {@const isPlayable =
          ...
          !isExpiredRecording &&
          recording.status &&
          !["deleted", "failed", "processing", "requested", "queued"].includes(
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div class="space-y-1">
                <div class="flex items-center space-x-2">
                  <h5
                    class="text-base font-semibold text-foreground truncate max-w-md"
                    title={recording.dvrHash}
                  >
                    {displayStreamId || recording.dvrHash}
                  </h5>
                  <span
                    class="text-xs px-2 py-1 rounded-full font-medium {recording.status?.toLowerCase() ===
                    'deleted'
                      ? 'bg-error/20 text-error'
                      : recording.status?.toLowerCase() === 'failed'
                        ? 'bg-error/20 text-error'
                ...
                <p class="text-xs text-muted-foreground">
                  Created: {recording.createdAt ? formatDate(recording.createdAt) : "N/A"} • Duration:
