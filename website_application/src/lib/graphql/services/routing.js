...
export const routingService = {
  ...
  async getRoutingEvents(streamId, timeRange) {
    try {
  ...
  async getConnectionEvents(streamId, timeRange) {
    try {
      const result = await client.query({
  ...
  async getNodeMetrics(nodeId, timeRange) {
    try {
      const result = await client.query({
  ...
  async getPlatformRoutingEvents(timeRange) {
    try {
      const result = await client.query({
  ...
  async getStreamConnectionEvents(streamId, timeRange) {
    try {
      const result = await client.query({
  ...
  async getAllNodeMetrics(timeRange) {
    try {
      const result = await client.query({
  ...
  async getRoutingEfficiency(streamId, timeRange) {
    try {
      ...
      const successfulRoutes = routingEvents.filter(event => event.status === 'success');
      const totalScore = routingEvents.reduce((sum, event) => sum + (event.score || 0), 0);
  ...
  async getConnectionPatterns(streamId, timeRange) {
    try {
      const connectionEvents = await this.getConnectionEvents(streamId, timeRange);
      
      const connections = connectionEvents.filter(event => event.eventType === 'connect');
      const disconnections = connectionEvents.filter(event => event.eventType === 'disconnect');
      
      // Group by country
      const countryConnections = connections.reduce((acc, event) => {
      ...
      // Group by node
      const nodeConnections = connections.reduce((acc, event) => {
  ...
  async getNodeHealthSummary(timeRange) {
    try {
      ...
      const latestMetrics = nodeMetrics.reduce((acc, metric) => {
        if (!acc[metric.nodeId] || new Date(metric.timestamp) > new Date(acc[metric.nodeId].timestamp)) {
          acc[metric.nodeId] = metric;
        }
        return acc;
      }, {});

      const healthCounts = Object.values(latestMetrics).reduce((acc, metric) => {
