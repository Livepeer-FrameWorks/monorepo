...
import { errorBoundary } from "$lib/stores/errorBoundary.ts";
...
const errorLink = onError(({ graphQLErrors, networkError }) => {
  ...
  if (graphQLErrors) {
    graphQLErrors.forEach(({ message, locations, path, extensions }) => {
      ...
      if (
        ...
        errorBoundary.handleGraphQLError({
  ...
  if (networkError) {
    ...
    const netError = networkError as NetworkErrorWithStatus;
    ...
    if (netError.statusCode && netError.statusCode >= 500) {
