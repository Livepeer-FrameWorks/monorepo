<script lang="ts">
  ...
  let maskedNodes = $derived(
    streamsEdges.map((e) => e.node).filter((node): node is NonNullable<typeof node> => node != null)
  );
  ...
  let totalViewers = $derived(
    streams.reduce((acc, s) => acc + (s?.metrics?.currentViewers || 0), 0)
  );
  ...
  function handleStreamEvent(event: StreamEventData) {
    ...
    streams = [...streams.slice(0, streamIndex), updatedStream, ...streams.slice(streamIndex + 1)];
  ...
  async function createStream() {
    ...
    try {
      ...
      const isError =
        createResult?.__typename === "ValidationError" || createResult?.__typename === "AuthError";
    ...
    <div class="flex justify-between items-center">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-muted-foreground">Manage your live streams and broadcasts</p>
        </div>
      </div>
    </div>
  </div>
                          ...
                          >
                            <div class="flex items-center gap-1">
                              ...
                              <Button
                                ...
                                title={expandedStreamId === stream.id
                                  ? "Hide Share Info"
                                  : "Share Stream"}
                                onclick={(e) => {
                                  e.stopPropagation();
                                  expandedStreamId =
                                    expandedStreamId === stream.id ? null : stream.id;
                                }}
                              >
                          ...
                          <TableCell class="px-4 py-2">
                            ...
                            >
                              {#if stream.metrics?.isLive}
                                <span
                                  class="w-1.5 h-1.5 rounded-full bg-success animate-pulse mr-1.5"
                                ></span>
                              {/if}
                              {status}
                            </span>
                            ...
                            {#if stream.metrics?.isLive}
                              <div class="flex flex-col gap-0.5">
                                ...
                                {#if stream.metrics?.qualityTier}
                                  <span
                                    class="text-[10px] px-1 py-0 bg-accent/10 text-accent border border-accent/20 rounded-sm w-fit"
                                  >
