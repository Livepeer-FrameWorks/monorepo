<script lang="ts">
  ...
  let geographicDistribution = $derived($geoDistStore.data?.analytics?.usage?.streaming?.geographicDistribution ?? null);
  ...
  let countryTrendData = $derived.by(() => {
    const hourlyEdges = $viewerGeoHourlyStore.data?.analytics?.usage?.streaming?.viewerGeoHourlyConnection?.edges ?? [];
  ...
  let countryUsageData = $derived.by(() => {
    ...
    const byCountry: Record<string, { viewerHours: number; egressGb: number; viewerCount: number }> = {};
  ...
  let totalViewerHours = $derived(
    countryUsageData.reduce((sum, c) => sum + c.viewerHours, 0)
  );
  let totalEgressGb = $derived(
    countryUsageData.reduce((sum, c) => sum + c.egressGb, 0)
  );
  ...
  async function loadAllData() {
    try {
      ...
      const connectionConnection = connectionEventsResult.data?.analytics?.lifecycle?.connectionEventsConnection;
  ...
  let mostPopularNodes = $derived.by(() => {
    const routingEdges = $routingEventsStore.data?.analytics?.infra?.routingEventsConnection?.edges ?? [];
            ...
            <div class="slab-body--padded space-y-4">
              ...
              {#if geographicDistribution.topCities.length}
                ...
                <div class="pt-3 border-t border-border/30">
                  <p class="text-xs text-muted-foreground uppercase tracking-wide mb-2">Top Cities</p>
                  <div class="space-y-2">
              ...
              <div class="flex items-center justify-between w-full">
                ...
                <div class="flex items-center gap-4 text-sm">
                  <span class="text-muted-foreground">
                    Total: <span class="font-semibold text-info">{totalViewerHours.toFixed(1)}h</span> viewing
                  </span>
                  <span class="text-muted-foreground">
              ...
              <table class="w-full text-sm">
                ...
                <tbody>
                  {#each countryUsageData as country (country.countryCode)}
                    {@const avgPerViewer = country.viewerCount > 0 ? (country.viewerHours / country.viewerCount * 60).toFixed(1) : '0'}
                    <tr class="border-b border-border/30 hover:bg-muted/10">
                      <td class="py-3 px-4 font-medium text-foreground">{getCountryName(country.countryCode)}</td>
                      <td class="py-3 px-4 text-right font-mono text-info">{country.viewerHours.toFixed(1)}h</td>
                      <td class="py-3 px-4 text-right font-mono text-primary">{country.egressGb.toFixed(2)}</td>
                      <td class="py-3 px-4 text-right font-mono">{country.viewerCount.toLocaleString()}</td>
                ...
                <tbody>
                  {#each geographicDistribution.topCities as city (`${city.city}-${city.countryCode}`)}
                    <tr class="border-b border-border/30 hover:bg-muted/10">
                      ...
                      <td class="py-3 px-4 text-muted-foreground">{getCountryName(city.countryCode ?? '')}</td>
                      <td class="py-3 px-4 text-right font-mono">{city.viewerCount.toLocaleString()}</td>
                      <td class="py-3 px-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                          <div class="w-16 h-1.5 bg-muted rounded-full overflow-hidden">
                            <div class="h-full bg-info" style="width: {Math.min(city.percentage || 0, 100)}%"></div>
                          </div>
                          <span class="font-mono text-xs w-12 text-right">{(city.percentage || 0).toFixed(1)}%</span>
                        </div>
                      </td>
                    </tr>
                  {/each}
