<script lang="ts">
  ...
  // Types from stores
  type ClipData = NonNullable<NonNullable<NonNullable<typeof $clipsStore.data>["clipsConnection"]>["edges"]>[0]["node"];
  type DvrData = NonNullable<NonNullable<NonNullable<typeof $dvrStore.data>["dvrRecordingsConnection"]>["edges"]>[0]["node"];
  type VodData = NonNullable<NonNullable<NonNullable<typeof $vodStore.data>["vodAssetsConnection"]>["edges"]>[0]["node"];
  ...
  let dvrRecordings = $derived($dvrStore.data?.dvrRecordingsConnection?.edges?.map(e => e.node) ?? []);
  ...
  let storageEvents = $derived(
    $storageEventsStore.data?.analytics?.lifecycle?.storageEventsConnection?.edges?.map(e => e.node) ?? []
  );
  ...
  let hasMoreItems = $derived.by(() => {
    ...
    return (clipsPageInfo?.hasNextPage || dvrPageInfo?.hasNextPage || vodPageInfo?.hasNextPage) ?? false;
  });
  ...
  let filteredArtifacts = $derived.by(() => {
    ...
    if (statusFilter !== "all") {
      result = result.filter(a => {
        ...
        if (statusFilter === "processing") return ["processing", "recording", "uploading", "requested"].includes(s);
  ...
  const lifecycleRangeOptions = TIME_RANGE_OPTIONS.filter(opt => ["24h", "7d", "30d"].includes(opt.value));
    ...
    try {
      ...
      if (typeFilter === 'clips' || typeFilter === 'all') {
        if (clipsPageInfo?.hasNextPage && clipsPageInfo.endCursor) {
          promises.push(clipsStore.fetch({ variables: { first: 50, after: clipsPageInfo.endCursor } }));
        }
      }
  ...
  function getStatusColor(status: string): string {
    ...
    if (["available", "completed", "ready"].includes(s)) return "text-success bg-success/10 border-success/20";
    if (["processing", "recording", "uploading", "requested"].includes(s)) return "text-warning bg-warning/10 border-warning/20";
  ...
  function canPlayArtifact(artifact: UnifiedArtifact): boolean {
    ...
    if (["processing", "recording", "uploading", "requested"].includes(artifact.status.toLowerCase())) return false;
    return true;
  }
        ...
        <div class="dashboard-grid">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <div class="flex border border-border rounded-md overflow-hidden">
                    <button
                      type="button"
                      class="flex-1 px-3 py-2 text-xs font-medium transition-colors {typeFilter === 'all' ? 'bg-primary text-primary-foreground' : 'bg-muted/30 text-muted-foreground hover:bg-muted/50'}"
                    ...
                    <button
                      type="button"
                      class="flex-1 px-3 py-2 text-xs font-medium transition-colors border-x border-border {typeFilter === 'clips' ? 'bg-primary text-primary-foreground' : 'bg-muted/30 text-muted-foreground hover:bg-muted/50'}"
                    ...
                    <button
                      type="button"
                      class="flex-1 px-3 py-2 text-xs font-medium transition-colors border-r border-border {typeFilter === 'dvr' ? 'bg-primary text-primary-foreground' : 'bg-muted/30 text-muted-foreground hover:bg-muted/50'}"
                    ...
                    <button
                      type="button"
                      class="flex-1 px-3 py-2 text-xs font-medium transition-colors {typeFilter === 'vod' ? 'bg-primary text-primary-foreground' : 'bg-muted/30 text-muted-foreground hover:bg-muted/50'}"
                ...
                <!-- Search -->
                <div>
                  <label for="search" class="block text-sm font-medium text-muted-foreground mb-2">Search</label>
                  <div class="relative">
                ...
                <!-- Status Filter -->
                <div>
                  <label for="status-filter" class="block text-sm font-medium text-muted-foreground mb-2">Status</label>
                  <Select bind:value={statusFilter} type="single">
                    <SelectTrigger id="status-filter" class="w-full">
                      {statusFilter === 'all' ? 'All Statuses' : statusFilter === 'processing' ? 'Processing' : statusFilter === 'ready' ? 'Ready' : 'Failed'}
                    </SelectTrigger>
                    <SelectContent>
              ...
              {#if filteredArtifacts.length === 0}
                <div class="p-8">
                  <EmptyState
                    ...
                    actionText={searchQuery || typeFilter !== 'all' || statusFilter !== 'all' ? "Clear Filters" : undefined}
                    onAction={searchQuery || typeFilter !== 'all' || statusFilter !== 'all' ? () => { searchQuery = ""; typeFilter = 'all'; statusFilter = 'all'; } : undefined}
                  />
                </div>
              {:else}
                <div class="overflow-x-auto">
                  <Table class="w-full">
                    <TableHeader>
                      <TableRow>
                        <TableHead class="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider w-[120px]">Actions</TableHead>
                        <TableHead class="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider w-[70px]">Type</TableHead>
                        <TableHead class="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Title</TableHead>
                        <TableHead class="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Source</TableHead>
                        <TableHead class="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Status</TableHead>
                        <TableHead class="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Duration</TableHead>
                        <TableHead class="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Size</TableHead>
                        <TableHead class="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Created</TableHead>
                        <TableHead class="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Expires</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody class="divide-y divide-border">
                      {#each filteredArtifacts as artifact (artifact.id)}
                        ...
                        <TableRow
                          class="transition-colors group {isDeleted || isExpiredArtifact ? 'opacity-60 bg-muted/30 cursor-not-allowed' : canPlayArtifact(artifact) ? 'hover:bg-muted/50 cursor-pointer' : 'cursor-default'}"
                          onclick={() => canPlayArtifact(artifact) && playArtifact(artifact)}
                        >
                          <!-- Actions -->
                          <TableCell class="px-4 py-2 align-middle" onclick={(e) => e.stopPropagation()}>
                            <div class="flex items-center gap-1">
                              {#if isExpiredArtifact}
                                <span class="text-[10px] text-muted-foreground px-2 italic">Expired</span>
                              {:else if isDeleted}
                                <span class="text-[10px] text-muted-foreground px-2 italic">Deleted</span>
                              {:else if canPlayArtifact(artifact) && artifact.playbackId}
                                {@const urls = getContentDeliveryUrls(artifact.playbackId, artifact.type === "clips" ? "clip" : artifact.type === "dvr" ? "dvr" : "vod")}
                                <Button
                                ...
                                <Button
                                  ...
                                  title="Delete"
                                  onclick={() => {
                                    if (artifact.type === "clips") confirmDeleteClip(artifact.rawData);
                                    else if (artifact.type === "dvr") confirmDeleteDvr(artifact.rawData);
                                    else if (artifact.type === "vod") confirmDeleteVod(artifact.rawData);
                                  }}
                                >
                          ...
                          <TableCell class="px-4 py-2">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border {getTypeColor(artifact.type)}">
                          ...
                          <TableCell class="px-4 py-2">
                            <div class="flex flex-col">
                              <div class="text-sm font-medium text-foreground truncate max-w-xs group-hover:text-primary transition-colors" title={artifact.title}>
                                {artifact.title}
                              </div>
                          ...
                          <TableCell class="px-4 py-2">
                            <div class="flex items-center gap-2 flex-wrap">
                              <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border {getStatusColor(artifact.status)}">
                                {artifact.status || "Unknown"}
                              </span>
                              {#if artifact.storageLocation === 'freezing'}
                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium border text-cyan-400 bg-cyan-400/10 border-cyan-400/20">
                              ...
                              {:else if artifact.isFrozen}
                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium border text-blue-400 bg-blue-400/10 border-blue-400/20">
                                  <SnowflakeIcon class="w-3 h-3" />
                                  Frozen
                                </span>
                              {:else if artifact.storageLocation === 'cloud' && artifact.type === 'dvr'}
                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium border text-emerald-400 bg-emerald-400/10 border-emerald-400/20">
                        ...
                        {#if expandedArtifact === artifact.id && canPlayArtifact(artifact) && artifact.playbackId}
                          <TableRow class="bg-muted/5 border-t-0">
                            <TableCell colspan={9} class="px-4 py-4 cursor-default" onclick={(e) => e.stopPropagation()}>
                              <div class="pl-4 border-l-2 border-primary/20">
                                <PlaybackProtocols
                                  contentId={artifact.playbackId}
                                  contentType={artifact.type === "clips" ? "clip" : artifact.type === "dvr" ? "dvr" : "vod"}
                                  showPrimary={true}
                                  showAdditional={true}
                                />
                              </div>
          ...
          <div class="slab col-span-full">
            <div class="slab-header">
              <div class="flex items-center gap-2">
                ...
                <h3>In Progress</h3>
                <span class="text-xs text-muted-foreground">({inProgressArtifacts.length} active)</span>
              </div>
            </div>
            <div class="slab-body--padded">
              <div class="space-y-3">
                {#each inProgressArtifacts as artifact (artifact.cursor)}
                  <div class="flex items-center gap-4 p-3 border border-border/30 bg-muted/20 rounded">
                    <div class="flex-1">
                      <div class="flex items-center gap-2 mb-1">
                        <span class="text-xs px-1.5 py-0.5 rounded font-mono {artifact.contentType === 'clip' ? 'bg-primary/20 text-primary' : artifact.contentType === 'dvr' ? 'bg-success/20 text-success' : 'bg-info/20 text-info'}">
          ...
          <div class="slab col-span-full">
            <div class="slab-header flex items-center justify-between">
              ...
              <Select value={lifecycleRange} onValueChange={(v) => lifecycleRange = v} type="single">
                <SelectTrigger class="min-w-[140px]">
                  {lifecycleRangeOptions.find(opt => opt.value === lifecycleRange)?.label ?? "Last 7 Days"}
                </SelectTrigger>
                <SelectContent>
            ...
            {#if mergedLifecycleEvents.length > 0}
                ...
                <Table>
                  ...
                  <TableBody>
                    {#each mergedLifecycleEvents.slice(0, lifecycleDisplayCount) as event, i (`${event.timestamp}-${event.eventKey}-${i}`)}
                      <TableRow>
                        <TableCell class="text-xs text-muted-foreground font-mono">{formatTimestamp(event.timestamp)}</TableCell>
                        <TableCell>
                          <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border {getTypeColor(event.type)}">
              ...
              {#if mergedLifecycleEvents.length > lifecycleDisplayCount}
                <div class="slab-actions">
                  <Button variant="ghost" class="w-full" onclick={() => lifecycleDisplayCount += 20}>
                    Load More Events
                  </Button>
                </div>
              {/if}
            {:else}
              <div class="slab-body--padded text-center">
                <p class="text-muted-foreground py-6">No lifecycle events in {lifecycleRangeResolved.label.toLowerCase()}</p>
              </div>
            {/if}
          </div>
                ...
                <TableBody>
                  {#each storageEvents as event (event.id)}
                    <TableRow>
                      ...
                      <TableCell>
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium {event.action === 'freeze' ? 'bg-info/20 text-info border border-info/30' : 'bg-warning/20 text-warning border border-warning/30'}">
...
<DeleteClipModal
  ...
  onCancel={() => { showDeleteClipModal = false; clipToDelete = null; }}
/>
...
<DeleteRecordingModal
  ...
  onConfirm={deleteDvr}
  onCancel={() => { showDeleteDvrModal = false; dvrToDelete = null; }}
/>

<!-- Delete VOD Modal -->
<Dialog open={showDeleteVodModal} onOpenChange={(v) => { if (!deletingVodId) { showDeleteVodModal = v; if (!v) vodToDelete = null; } }}>
  <DialogContent class="max-w-sm rounded-none border-[hsl(var(--tn-fg-gutter)/0.3)] bg-background p-0 gap-0 overflow-hidden">
    <DialogHeader class="slab-header text-left space-y-1">
      ...
      <DialogDescription class="text-xs text-muted-foreground/70">This action cannot be undone.</DialogDescription>
    </DialogHeader>
    <div class="slab-body--padded">
      <p class="text-sm text-foreground">Are you sure you want to delete <strong>{vodToDelete?.title || vodToDelete?.filename || "this video"}</strong>?</p>
    </div>
    <DialogFooter class="slab-actions slab-actions--row gap-0">
      <Button type="button" variant="ghost" class="rounded-none h-12 flex-1 border-r border-[hsl(var(--tn-fg-gutter)/0.3)]" onclick={() => { showDeleteVodModal = false; vodToDelete = null; }} disabled={!!deletingVodId}>Cancel</Button>
      <Button type="button" variant="ghost" class="rounded-none h-12 flex-1 text-destructive" onclick={deleteVod} disabled={!!deletingVodId}>{deletingVodId ? "Deleting..." : "Delete"}</Button>
    </DialogFooter>
  </DialogContent>
</Dialog>
...
<Dialog open={showCreateClipModal} onOpenChange={(v) => (showCreateClipModal = v)}>
  <DialogContent class="max-w-md rounded-none border-[hsl(var(--tn-fg-gutter)/0.3)] bg-background p-0 gap-0 overflow-hidden">
    <DialogHeader class="slab-header text-left space-y-1">
      ...
      <DialogDescription class="text-xs text-muted-foreground/70">Choose a stream and time range.</DialogDescription>
    </DialogHeader>

    <form id="create-clip-form" class="slab-body--padded space-y-4" onsubmit={(e) => { e.preventDefault(); createClip(); }}>
      <!-- Mode Tabs -->
      <div class="space-y-2">
        <span class="block text-sm font-medium text-muted-foreground mb-2">Clipping Mode</span>
        <div class="flex border border-border rounded-md overflow-hidden">
          <button type="button" class="flex-1 px-3 py-2 text-sm font-medium transition-colors {clipMode === 'CLIP_NOW' ? 'bg-primary text-primary-foreground' : 'bg-muted/30 text-muted-foreground hover:bg-muted/50'}" onclick={() => clipMode = 'CLIP_NOW'}>Clip Now</button>
          <button type="button" class="flex-1 px-3 py-2 text-sm font-medium transition-colors border-x border-border {clipMode === 'DURATION' ? 'bg-primary text-primary-foreground' : 'bg-muted/30 text-muted-foreground hover:bg-muted/50'}" onclick={() => clipMode = 'DURATION'}>Duration</button>
          <button type="button" class="flex-1 px-3 py-2 text-sm font-medium transition-colors {clipMode === 'ABSOLUTE' ? 'bg-primary text-primary-foreground' : 'bg-muted/30 text-muted-foreground hover:bg-muted/50'}" onclick={() => clipMode = 'ABSOLUTE'}>Timestamps</button>
        </div>
      </div>

      <div class="space-y-2">
        <label for="stream-select" class="block text-sm font-medium text-muted-foreground mb-2">Stream</label>
        <Select bind:value={selectedStreamId} type="single">
          <SelectTrigger id="stream-select" class="w-full">
            <span class={selectedStreamId ? "" : "text-muted-foreground"}>{selectedStreamLabel}</span>
          </SelectTrigger>
          <SelectContent>
            {#if streams.length === 0}
      ...
      <div class="space-y-2">
        <label for="clip-title" class="block text-sm font-medium text-muted-foreground mb-2">Title</label>
        <Input id="clip-title" type="text" bind:value={clipTitle} placeholder="Enter clip title" required />
      </div>

      <div class="space-y-2">
        <label for="clip-description" class="block text-sm font-medium text-muted-foreground mb-2">Description (optional)</label>
        <Textarea id="clip-description" bind:value={clipDescription} placeholder="Enter clip description" rows={2} />
      </div>
        ...
        <div class="space-y-2">
          ...
          <div class="flex gap-2">
            {#each durationPresets as preset (preset.value)}
              <button type="button" class="flex-1 px-3 py-2 text-sm font-medium rounded border transition-colors {clipDuration === preset.value ? 'bg-primary text-primary-foreground border-primary' : 'bg-muted/30 text-muted-foreground border-border hover:bg-muted/50'}" onclick={() => clipDuration = preset.value}>{preset.label}</button>
            {/each}
          </div>
          <p class="text-xs text-muted-foreground/70">Captures the last {formatDuration(clipDuration)} from the live stream</p>
        </div>
      {:else if clipMode === 'DURATION'}
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-2">
            <label for="start-time" class="block text-sm font-medium text-muted-foreground">Start Time (unix)</label>
            <Input id="start-time" type="number" bind:value={clipStartTime} min="0" required />
          </div>
          <div class="space-y-2">
            <label for="duration-input" class="block text-sm font-medium text-muted-foreground">Duration (seconds)</label>
      ...
      {:else}
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="space-y-2">
            <label for="end-time" class="block text-sm font-medium text-muted-foreground">End Time (unix)</label>
    ...
    <DialogFooter class="slab-actions slab-actions--row gap-0">
      <Button type="button" variant="ghost" class="rounded-none h-12 flex-1 border-r border-[hsl(var(--tn-fg-gutter)/0.3)]" onclick={() => (showCreateClipModal = false)} disabled={creatingClip}>Cancel</Button>
      <Button type="submit" variant="ghost" class="rounded-none h-12 flex-1 text-primary" disabled={creatingClip || !selectedStreamId} form="create-clip-form">{creatingClip ? "Creating..." : "Create Clip"}</Button>
    </DialogFooter>
  </DialogContent>
</Dialog>

<!-- Upload VOD Modal -->
<Dialog open={showUploadModal} onOpenChange={(v) => { if (!uploading) { showUploadModal = v; if (!v) resetUploadForm(); } }}>
  <DialogContent class="max-w-md rounded-none border-[hsl(var(--tn-fg-gutter)/0.3)] bg-background p-0 gap-0 overflow-hidden">
    <DialogHeader class="slab-header text-left space-y-1">
      <DialogTitle class="uppercase tracking-wide text-sm font-semibold text-muted-foreground">Upload Video</DialogTitle>
      <DialogDescription class="text-xs text-muted-foreground/70">Upload a video file to your library.</DialogDescription>
    </DialogHeader>

    <form id="upload-form" class="slab-body--padded space-y-4" onsubmit={(e) => { e.preventDefault(); startUpload(); }}>
      <div class="space-y-2">
        <label for="file-input" class="block text-sm font-medium text-muted-foreground mb-2">Video File</label>
        <div class="relative border-2 border-dashed border-border rounded-lg p-6 text-center hover:border-primary/50 transition-colors">
          ...
          {/if}
          <input id="file-input" type="file" accept="video/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onchange={handleFileSelect} disabled={uploading} />
        </div>
      </div>

      <div class="space-y-2">
        <label for="upload-title" class="block text-sm font-medium text-muted-foreground mb-2">Title</label>
        <Input id="upload-title" type="text" bind:value={uploadTitle} placeholder="Enter video title" disabled={uploading} />
      </div>

      <div class="space-y-2">
        <label for="upload-description" class="block text-sm font-medium text-muted-foreground mb-2">Description (optional)</label>
        <Textarea id="upload-description" bind:value={uploadDescription} placeholder="Enter video description" rows={2} disabled={uploading} />
      </div>
      ...
      {#if uploading}
        <div class="space-y-2">
          <div class="flex justify-between text-xs text-muted-foreground">
            <span>
              {#if uploadStage === "initializing"}Initializing...{:else if uploadStage === "uploading"}Uploading... {uploadProgress}%{:else if uploadStage === "completing"}Completing...{:else if uploadStage === "done"}Complete!{/if}
            </span>
    ...
    <DialogFooter class="slab-actions slab-actions--row gap-0">
      <Button type="button" variant="ghost" class="rounded-none h-12 flex-1 border-r border-[hsl(var(--tn-fg-gutter)/0.3)]" onclick={cancelUpload} disabled={uploading && uploadStage === "completing"}>{uploading ? "Cancel" : "Close"}</Button>
      <Button type="submit" variant="ghost" class="rounded-none h-12 flex-1 text-primary" disabled={uploading || !uploadFile} form="upload-form">{uploading ? "Uploading..." : "Upload"}</Button>
    </DialogFooter>
  </DialogContent>
</Dialog>
