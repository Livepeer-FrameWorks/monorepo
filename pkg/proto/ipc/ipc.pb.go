...
func (x *ControlMessage) GetGoingAway() *GoingAway {
	if x != nil {
		if x, ok := x.Payload.(*ControlMessage_GoingAway); ok {
			return x.GoingAway
		}
	}
	return nil
...
type ControlMessage_GoingAway struct {
	// Foghorn is shutting down. Helmsman stops sending on this stream, lets
	// in-flight requests finish, closes it, and redials at once: the address
	// resolves to the cell's other instances, and this one has already closed
	// its listener. A Helmsman that predates the message ignores it and
...
func (x *GoingAway) String() string {
	return protoimpl.X.MessageStringOf(x)
}
