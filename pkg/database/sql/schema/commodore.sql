...
CREATE TABLE IF NOT EXISTS commodore.clips (
    ...
    -- Monotonic source revision of the last applied catalog snapshot. Foghorn advances this
    -- per artifact from the row's prior revision whenever catalog-relevant state changes — it
    -- is NOT a timestamp or a globally ordered sequence. The projection RPC applies only if
    -- the incoming revision is newer for this artifact, so a stale reconciler snapshot or a
    -- non-authoritative cluster can't regress newer state.
...
-- advanced per artifact and delivered by the reconciler's delete projection — the single writer for
-- that artifact's revision. It exists INDEPENDENTLY of the business row, so a delete that lands before
...
ALTER TABLE commodore.vod_assets ALTER COLUMN requires_auth SET DEFAULT TRUE;
...
-- DVRs snapshot their parent stream policy so playback authority remains
-- resolvable after the live stream is deleted or the control plane is offline.
...
CREATE INDEX IF NOT EXISTS idx_media_authority_refresh_inbox_due_v2
    ON commodore.media_authority_refresh_inbox(next_attempt_at ASC, created_at ASC)
    WHERE status <> 'completed';

CREATE INDEX IF NOT EXISTS idx_media_authority_refresh_inbox_completed_v2
