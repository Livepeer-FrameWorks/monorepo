...
CREATE TABLE IF NOT EXISTS quartermaster.infrastructure_clusters (
    ...
    -- Revision zero preserves unconfigured owner permissions, not tenant entitlement.
    media_consent_revision BIGINT NOT NULL DEFAULT 0,
    media_allow_ingest BOOLEAN NOT NULL DEFAULT TRUE,
    media_allow_serve BOOLEAN NOT NULL DEFAULT TRUE,
    media_allow_external_source BOOLEAN NOT NULL DEFAULT TRUE,
    CONSTRAINT chk_cluster_media_consent_revision CHECK (media_consent_revision >= 0),
...
CREATE TABLE IF NOT EXISTS quartermaster.node_fingerprints (
    ...
    node_identity_public_key_ed25519 BYTEA,
    seen_ips INET[] DEFAULT '{}',
    attrs JSONB DEFAULT '{}',
    first_seen TIMESTAMP DEFAULT NOW(),
    last_seen TIMESTAMP DEFAULT NOW(),
    CONSTRAINT chk_qm_node_identity_public_key
        CHECK (node_identity_public_key_ed25519 IS NULL OR octet_length(node_identity_public_key_ed25519) = 32)
);
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_qm_fingerprints_machine
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_qm_fingerprints_macs
...
CREATE TABLE IF NOT EXISTS quartermaster.service_event_outbox (
    ...
    -- The ID every dispatch sends. A service event with a domain counterpart
    -- carries the domain event's ID; rows without one dispatch under id.
    event_id     UUID,
...
-- ============================================================================
-- DOMAIN EVENT OUTBOX
-- pkg/events/outbox.TableDDL("quartermaster"), verbatim. Tenant, cluster,
-- invite, and cluster subscription domain events commit here in the state
-- change's transaction; the relay delivers them to Decklog PublishDomainEvents.
...
CREATE TABLE IF NOT EXISTS quartermaster.navigator_tenant_alias_outbox (
    ...
    cluster_id TEXT,
    reason     TEXT,
    action     TEXT NOT NULL CHECK (action IN ('ensure', 'ensure_cluster', 'retire', 'remove', 'remove_cluster')),
    created_at   TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    claimed_at   TIMESTAMPTZ,
    ...
    last_error   TEXT,
    completed_at TIMESTAMPTZ,
    -- ensure/retire need a non-blank label; cluster actions need a non-blank
    ...
    CONSTRAINT chk_alias_outbox_cluster   CHECK (action NOT IN ('ensure_cluster', 'remove_cluster') OR NULLIF(btrim(cluster_id), '') IS NOT NULL)
);
...
AFTER INSERT OR UPDATE OR DELETE ON quartermaster.tenant_cluster_access
...
-- Receipts retain the authorizing owner; a subsequent owner cannot recover their commands.
