...
BEGIN
    SELECT array_agg(expected.index_name ORDER BY expected.index_name)
      ...
      FROM (VALUES
          ('idx_purser_usage_records_allowance'::NAME, 'purser.usage_records'::regclass,
              ARRAY['tenant_id', 'usage_type', 'value_kind', 'granularity', 'period_start', 'period_end']::NAME[]),
          ('idx_usage_adjustments_allowance'::NAME, 'purser.usage_adjustments'::regclass,
              ARRAY['tenant_id', 'usage_type', 'value_kind', 'status', 'period_start', 'period_end']::NAME[])
      ) AS expected(index_name, table_oid, key_columns)
      LEFT JOIN pg_namespace index_namespace
        ON index_namespace.nspname = 'purser'
    ...
    IF invalid_indexes IS NOT NULL THEN
        RAISE EXCEPTION 'billing allowance indexes are absent or invalid: %',
