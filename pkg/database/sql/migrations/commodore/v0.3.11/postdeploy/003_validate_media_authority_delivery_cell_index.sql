...
BEGIN
    SELECT array_agg(expected.index_name ORDER BY expected.index_name)
      ...
      FROM (VALUES
          ('idx_media_authority_deliveries_cell_due'::NAME, 'commodore.media_authority_deliveries'::regclass,
              ARRAY['cell_id', 'replay', 'next_attempt_at', 'created_at']::NAME[], TRUE,
              'NOTshort_leaseANDstatus=ANYARRAY[''pending'',''delivering'']'),
          ('idx_media_authority_deliveries_rejected'::NAME, 'commodore.media_authority_deliveries'::regclass,
              ARRAY['authority_kind', 'authority_id']::NAME[], TRUE, 'status=''rejected''')
