...
const IconList = forwardRef(
  ({ items = [], tone = 'accent', columns = 1, stackAt = 'md', variant = 'card', indicator = 'icon', className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'marketing-icon-list',
    ...
    >
      {items.map((item, index) => (
        ...
        >
          ...
          {item.description ? <p className="marketing-icon-list__description">{item.description}</p> : null}
        </div>
      ))}
    </div>
  )
