  ...
  (
    ...
    const buildWallCard = (item) => {
      ...
      const titleRowClass = cn(
        "marketing-feature__title-row",
        tileVariant && "marketing-feature__title-row--tile"
      );
      const headerClass = cn(
        "marketing-feature__header",
        tileVariant && "marketing-feature__header--tile"
      );
      const iconClass = cn(
      ...
      return (
        <div
          className={cn(
            "marketing-feature",
            toneClass,
            tileVariant && "marketing-feature--tile",
            !shouldShowStripe && "marketing-feature--no-stripe"
          )}
        >
          <div className={headerClass}>
            <div className={titleRowClass}>
              ...
              <div className="marketing-feature__heading">
                ...
                {item.badge ? (
                  <span className="marketing-feature__subtitle">{item.badge}</span>
                ) : null}
              </div>
            </div>
            {item.meta ? (
              <div className="marketing-feature__meta">{renderSlot(item.meta)}</div>
            ) : null}
          </div>
    ...
    const renderSeamedNode = (item, index, { flushMode = false } = {}) => {
      const seamClasses = ["marketing-feature__cell", flushMode && "marketing-feature-grid__cell"];
      ...
      const hasRight =
        effectiveColumns > 1 && colPosition < effectiveColumns && index + 1 < totalItems;
    ...
    if (useWallLayout) {
      return (
        <div
          ref={ref}
          className={cn(
            "marketing-feature-wall",
            wallResponsiveClasses,
