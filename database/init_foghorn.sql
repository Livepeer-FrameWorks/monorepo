...
-- Storage management policies per node/tenant
CREATE TABLE IF NOT EXISTS foghorn.storage_policies (
    -- ===== IDENTITY =====
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    node_id VARCHAR(100),        -- NULL = global policy
    tenant_id UUID,              -- NULL = all tenants
    
    -- ===== RETENTION POLICIES =====
    min_retention_hours INTEGER DEFAULT 24,
    max_storage_percent INTEGER DEFAULT 80,
    cleanup_threshold_percent INTEGER DEFAULT 90,
    priority_rules JSONB DEFAULT '["age", "access_count"]', -- Cleanup priority
...
CREATE INDEX IF NOT EXISTS idx_foghorn_storage_policies_node ON foghorn.storage_policies(node_id);
CREATE INDEX IF NOT EXISTS idx_foghorn_storage_policies_tenant ON foghorn.storage_policies(tenant_id);
CREATE INDEX IF NOT EXISTS idx_foghorn_storage_policies_active ON foghorn.storage_policies(is_active);
