...
func (runtime *LivePushPreparationRuntime) Revalidate(ctx context.Context, req *placementpb.PreparePlacementRequest, receipt PlacementReceipt) error {
	...
	if receipt.Response != nil {
		...
		if outputsErr != nil || receipt.Response.GetReady() || receipt.Response.GetEndpoint() != observed.endpoint || receipt.Response.GetPublicBaseUrl() != observed.node.Host || receipt.Response.GetOutputsJson() != outputsJSON || receipt.Response.GetExpiresAt().AsTime().After(observed.expiresAt) {
