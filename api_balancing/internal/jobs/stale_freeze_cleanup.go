...
func (j *StaleFreezeCleanupJob) cleanup() {
		...
		for _, a := range stale {
			...
			for _, key := range []string{
				...
				if eErr := control.EnqueueStagingCleanupTx(ctx, tx, key); eErr != nil {
					stage, failedAttemptID = "enqueue", a.attemptID
					return eErr
				}
			}
		}
	...
	if err != nil {
		...
		case "enqueue":
			j.logger.WithError(err).WithField("attempt_id", failedAttemptID).Warn("Failed to enqueue abandoned freeze cleanup")
		default:
			j.logger.WithError(err).Warn("Failed to commit stale freeze recovery")
		}
		return
	}
	if len(stale) > 0 {
