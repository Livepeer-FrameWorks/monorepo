...
3. Federation address convention: `foghorn.{cluster_slug}.{base_url}:18029`. Navigator publishes this name in the Bunny cluster zone and issues the matching `cluster:{cluster_slug}` TLS bundle with both the cluster apex and wildcard SANs. Federation uses Foghorn's external gRPC listener because `PeerManager.NotifyPeers` consumes `TenantClusterPeer.foghorn_grpc_addr` from Quartermaster and falls back to this cluster FQDN shape when the address is absent.
