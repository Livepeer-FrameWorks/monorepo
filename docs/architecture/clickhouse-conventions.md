...
`ledger_rebuild_cursors_v2` (one row per ledger, `ReplicatedReplacingMergeTree(last_processed_projection_ms)`, `ORDER BY ledger_name`) tracks each rebuild worker's position: the workers in `api_analytics_ingest/internal/handlers/ledger_rebuilders.go` walk source facts by `projection_version_ms` and recompute every affected 5-minute window, appending new projection rows to the ledger. Both replacement and runtime reads use the monotonic processed watermark, so a delayed writer cannot move a cursor backwards merely because it committed later. The legacy `ledger_rebuild_cursors` table remains untouched for mixed-version rollback safety but is no longer read or written by v0.3.0 binaries. An operator-requested replay must first remove the ledger's v2 cursor rows with the supported ClickHouse maintenance procedure; appending a lower watermark is intentionally ignored. Restarts resume from the cursor; recomputation is idempotent because ledgers use the same append + `argMax`-on-read projection model. The delivery worker alone projects playback and restream final facts into `delivery_usage_5m`; its 90-day bootstrap uses one-hour scan chunks and bounded emission batches.
