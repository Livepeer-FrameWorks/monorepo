...
- **Unit** — the physical unit Purser stores and rating multiplies by `unit_price_per_<unit>`. Display unit on invoices/dashboards may differ (e.g. GiB-seconds internally, GiB-months rated and displayed).
...
| ------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------- | --------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `delivered_minutes`                                                                                                                        | minutes                                                   | yes                                     | Mist `USER_END` or sanitized `RESTREAM_STATUS_FINAL`                                                | Playback: `periscope.viewer_sessions_final`; restream: `periscope.restream_sessions_final`. Both project to `delivery_usage_5m`; `viewer_usage_5m` remains audience-only.                                                                                    | `billable_at_ms` (= `min(projection_version_ms)`, derived; cursor uses settlement lag default 2 min)                               | Playback or restream attempt `source_started_at_ms` .. `source_ended_at_ms` (half-open); restream emits `delivery_kind=restream` plus bounded `platform`, while playback retains its dimension-free legacy identity | Playback without `USER_END` → `viewer_sessions_anomalous`; restream without `RESTREAM_STATUS_FINAL` → `restream_sessions_anomalous`. Stale-close rows are never rated. A valid final with unobserved bytes still bills its independently observed duration.                                  |
| `egress_gb`                                                                                                                                | GiB delivered                                             | no (priceable)                          | Mist `USER_END` or sanitized `RESTREAM_STATUS_FINAL`                                                | Playback `downloaded_bytes` in `viewer_sessions_final`; restream `bytes_sent` in `restream_sessions_final`                                                                                                                                                   | `billable_at_ms`                                                                                                                   | Same delivery interval; restream uses `delivery_kind=restream` plus bounded `platform`; absent `delivery_kind` is playback for cross-version replay compatibility                                                   | Missing restream byte observations are recorded as `bytes_unobserved`, never estimated, and contribute zero bytes without suppressing the independently observed duration. Custom/marketplace pricing can opt in without changing the usage shape.                                           |
...
Restream rating begins at the release-defined cutover
**2026-09-04T00:00:00Z**. The gate applies to the logical fact's first
`projection_version_ms`, not its source end time, and the same gate excludes
later corrections to pre-cutover facts. Only sanitized
terminal (`idle` or `failed`) `RESTREAM_STATUS_FINAL` projections qualify. A
nonterminal final is rejected by Foghorn and quarantined by Periscope rather
than entering a billable fact table. Historical raw `PUSH_END` rows are
never backfilled or rated because they lack the fenced byte-observation contract
and may contain destination credentials.
...
This model applies to every `*_final` and 5-minute ledger table — viewer sessions, restream sessions, stream sessions, processing segments, and the six 5-min ledgers. The architecture is identical; only the field shapes differ.
...
Every ledger rebuild and stale-close pass holds a named, session-scoped
PostgreSQL advisory lock. This elects one Periscope process rather than one
ClickHouse server, and connection or process loss releases the lock for the next
replica. Workers expose a bounded leader gauge and cursor-lag gauge so a missing
writer or a stalled cursor is visible before a migration gate depends on it.
Delivery retractions retain a scalar natural-key predicate even when the
source-time predicate is deliberately relaxed, allowing the table's bloom
filter to bound late tombstone scans.
...
- Every refreshable view fed directly by an append ledger discovers affected natural keys from the raw table, where zero-valued tombstones remain visible, and computes replacement values from the deduplicated `*_v` surface. Each selector's source-time horizon stops one day inside that source table's own TTL; there is no global 90-day horizon (`api_usage_5m` retains 365 days and finalized viewer facts retain 730 days).
- Upgrade discovery markers expire by TTL rather than being truncated after a manual refresh. That lets an already-running scheduled refresh finish against the same discovery set; replacement refresh queries carry their own external group/sort spill limits because session settings do not propagate to scheduled executions.
...
The two tenant-daily families intentionally answer different questions.
`tenant_viewer_daily` is an audience/playback surface, so its egress comes only
from viewer delivery. `tenant_analytics_daily` and the platform daily-stats API
are delivery-capacity surfaces, so their egress includes playback plus
restream. Neither surface lets restream attempts contribute human-viewer or
geography counts.
...
- Write an audit row to `periscope.projection_divergences` with the values and source event. Restream divergences carry a stable `occurrence_id`; that identity is also embedded as `_correction_occurrence` in `natural_key_json`, which keeps old and new billing readers on one hash during a rolling upgrade. Reprocessing that row retains its identity, while a later restream `A→B→A→B` flap receives a distinct additive adjustment. Older playback, processing, stream-runtime, and storage correction classes keep an empty occurrence and their pre-v0.3.0 natural-key bytes so historical replays retain the established adjustment identity.
...
| `storage_gb_seconds_*`                                    | Rule-rated unit: catalog GiB-months (`gb_seconds / 2,628,000`, a fixed 730-hour month); unmarked existing rules GiB-hours (`/ 3,600`) |
...
4. Pricing-row migration or explicit per-rule unit config if the rated unit
   changes. Persisted rules without `config.rated_quantity_divisor` remain
   GiB-hour rules; new GiB-month rules must state both that divisor and
   `config.rated_unit`. Repricing a stored rule requires restating its
   `unit_price` and `included_quantity` in the new unit.
