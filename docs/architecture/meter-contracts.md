...
| `llm_input_tokens`, `llm_output_tokens`, `embedding_tokens`, `embedding_requests`, `search_requests` | tokens / requests | Skipper's durable `skipper_usage` publisher through Decklog | `periscope.api_usage_5m`                                                                           | `window_start`                                 | Persisted and itemized even when the price is zero. Provider/model/service are bounded dimensions.                                       |
