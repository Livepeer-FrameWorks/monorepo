...
- **Loss-tolerant telemetry**: the event is observational and a consumer can survive missing one. Loss = a gap in a graph. Examples: API request counters, heartbeats, best-effort Mist health/track/thumbnail triggers, routing decisions (aggregated stats).
...
| `internal/triggers/processor.go:768`                                                                     | Mist trigger forwarding                        | **mixed**         | Eight final/accounting triggers use Helmsman's WAL after local HTTP acceptance; STREAM_BUFFER, LIVE_TRACK_LIST, THUMBNAIL_UPDATED, and PROCESS_EXIT remain best effort. Mist does not replay a failed asynchronous HTTP delivery. See `trigger-durability.md`. |
