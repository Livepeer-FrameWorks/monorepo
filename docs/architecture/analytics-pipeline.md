...
For terminal accounting trigger classes (`USER_END`, `STREAM_END`, `PUSH_END`, `PUSH_INPUT_CLOSE`, `RECORDING_END`, `RECORDING_SEGMENT`, `LIVEPEER_SEGMENT_COMPLETE`, `PROCESS_AV_VIRTUAL_SEGMENT_COMPLETE`), Helmsman persists a safe parsed record to a local write-ahead log before responding 200 OK to Mist, and a background forwarder retries until Foghorn returns a `MistTriggerAck` after Decklog's Kafka publish commits. Managed live-restream `PUSH_END` is first correlated to target identity and replaced by a sanitized `RESTREAM_STATUS_FINAL`; its credential-bearing URI and Mist logs never enter the WAL or analytics pipeline. `USER_NEW` is a blocking admission trigger instead: Helmsman gives it the same deterministic identity and Mist is not allowed to establish the connection until Foghorn has synchronously committed the source fact to Kafka. See [trigger-durability.md](trigger-durability.md). The source id used for ack correlation hashes node, trigger type, raw payload, and the preferred Mist natural key (`X-Trigger-UUID` plus `X-Trigger-UnixMillis`) into `MistTrigger.RequestId`; only the derived ID is retained for sanitized restream finals. Decklog derives the typed Kafka `event_id` from that source id as a deterministic UUID so Periscope's UUID-based fact tables stay idempotent.
...
Decklog's full contract — the three gRPC RPCs, tenant fail-closed validation, deterministic `event_id` derivation, the raw-trigger audit topic, envelope region/cluster backfill, and the DLQ/replay model — is documented in [decklog.md](decklog.md).
...
The shared Kafka consumer behavior lives in `pkg/kafka`. Transient dependency failures retry in place without committing; permanent handler failures are captured to the `decklog_events_dlq` dead-letter topic before the offset commits — see [decklog.md](decklog.md) for the DLQ + replay contract.
...
Foghorn tracks which nodes hold a local copy of each artifact in `foghorn.artifact_nodes` (media-plane-local). The media plane reads this table directly for routing/serving/relay, but it lives in each cluster's own Foghorn DB and is **not** reachable from the analytics/read surfaces (Bridge never queries the media plane), so it is emitted from `api_balancing/internal/control/repos.go` into this analytics pipeline for cross-cluster, dashboard-facing reads.

**Scope: transient local node copies, not durable storage.** This reports which nodes currently hold a _local copy_ of an artifact — the producer/origin copy or a synced cache copy — as an eventually-consistent projection. It is **not** the durable copy: today the one durable copy lives in object storage (S3), attributed to the artifact's `storageClusterId` and gated on its sync/frozen lifecycle, tracked via the artifact's storage state, not here. Node copies are transient (evicted/orphaned by cleanup). The asset analytics UI composes the two into a "Storage & copies" view: one durable S3 location plus N local node copies. Generalizing the singleton durable location into backend-keyed placement and a tier ladder is a deferred follow-up tracked in `docs/rfcs/placement-policy-engine.md`; node-copy telemetry stays a separate concern.
