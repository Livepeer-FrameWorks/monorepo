...
| Input                                                                                                                                                                                      | Why                                                                                                |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------- |
| Files in the import closure of `release-components.json:.cmd` for the service, restricted to monorepo-internal packages                                                                    | The actual source compiled into the binary (incl. the committed generated `*.pb.go`)               |
| The exact `.proto` source closure: the `.proto` behind each generated proto package in the import closure, plus their transitive proto imports; **never every `.proto` under `pkg/proto`** | Defense-in-depth on the generated code, without a one-proto edit invalidating every proto consumer |
...
Test files (`*_test.go`) are excluded; they don't affect the binary. The import closure comes from `go list -deps -json <cmd>`, run from the service's module root, and is the **union across every platform the component ships**: `linux/amd64` and `linux/arm64`, plus `darwin/arm64` when `darwin_binary` is set. This keeps platform-specific files (e.g. `*_darwin.go`) in the hash so a single-platform change can't be carried forward incorrectly. CGO is enabled only for components marked `cgo: true` in `.github/release-components.json`. A file-target `cmd` (`cmd/x/main.go`, which `go list` reports as the synthetic `command-line-arguments` package) still has its entrypoint hashed.
