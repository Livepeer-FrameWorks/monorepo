...
Control-plane unavailability is not a healthy billing verdict. Quartermaster explicitly marks billing status unavailable when Purser cannot answer; it no longer fabricates a postpaid, unsuspended tenant. Foghorn represents tenant billing state as `healthy`, `denied`, `stale-valid`, or `unavailable`, keyed by tenant rather than only by stream. A still-valid local entry continues to serve while refresh fails; a hard miss or expired entry is unavailable, not an unmetered allow.
