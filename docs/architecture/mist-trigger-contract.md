...
A Foghorn restart stays inside that budget. On shutdown Foghorn holds the health of the nodes connected to it for the reconnect window (they are healthy and about to reconnect, so they are neither marked unhealthy nor withdrawn from DNS), closes its listeners, tells each node it is going away, keeps answering the requests already in flight for up to three seconds, and only then ends the streams and stops its trigger processor. Helmsman, on that notice, sends nothing more on the stream — a new blocking trigger waits in the reconnect grace like through any reconnect, everything else goes to its outbox — lets in-flight requests be answered, and redials within a quarter of a second, then about once a second for twenty seconds instead of backing off. The address resolves to the rest of the cell because the instance that sent the notice has already closed its listener; in a cell of one it is the same instance coming back. A Helmsman that predates the notice ignores it and reconnects when the stream ends.
...
| `PROCESS_REPLACE` | Replace a process that exited unrecoverably on a `live+`, `pull+` or `processing+` buffer. | `value`                                      | none      | Helmsman answers locally, without a Foghorn round trip. A failed `Livepeer` process is replaced by the equivalent local MistProcAV ladder; every other process gets `[]` and stays disabled. Mist asks at most once per failed config per buffer. Helmsman then reports `stream_transcode_degraded` to Foghorn, which logs it, counts it, and stamps the live ingest session. |
...
| `PROCESS_EXIT`                        | Process exit signal           | Best effort; configured with `streams: ["live+", "pull+", "processing+"]`; see below                                  |
...
filter: it covers the stream families that run FrameWorks processes. Exits on a
`processing+` stream are routed to that job's in-memory listener; a missing
listener or a full queue is logged and counted. Exits on `live+` and `pull+`
streams are logged as telemetry with their status and reason.
