...
Foghorn
  ...
  → calls Chandler's service-authenticated cache invalidation endpoint:
    - `CHANDLER_INTERNAL_URL` generated by provisioning as comma-separated per-instance mesh URLs
      such as `http://central-eu-1.internal:18020,http://regional-eu-1.internal:18020`
    - otherwise the managed Chandler base URL used for playback metadata
```
...
- `POST /internal/assets/cache/invalidate` is service-token authenticated and intended
  for Foghorn → Chandler calls, not player traffic
