...
Drafts are stored in `skipper.skipper_posts` (`internal/social/store.go`), scoped to the deployment's Quartermaster-owned system tenant (`SYSTEM_TENANT_ID`, validated at startup through `config.SystemTenant`): content type, tweet text, context summary, trigger data (JSONB), and status (`draft` → `sent` once the notification email goes out). The store also backs the daily-limit count and the recent-post lists used for deduplication and theme avoidance.
