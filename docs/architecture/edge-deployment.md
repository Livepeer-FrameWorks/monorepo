...
MistServer is special because most runtime work is process-per-session. A new Mist release can rebuild every binary and still not require a node drain. The default Mist apply strategy is therefore `rolling_stage`: Helmsman downloads and verifies the complete Mist bundle, stages a rollback copy, atomically replaces each release-owned file inside the stable Mist install directories, and installs `MistController` last. Keeping the canonical `bin/` directory in place is required by Mist's built-in rolling restart, which locates replacement executables through `/proc/self/exe`. Helmsman then sends `USR1` only to the controller angel/main process. The controller preserves and re-adopts its protocol listeners, and each listener rolls itself while its detached per-connection children continue. Existing live inputs and processing workers are not controller-owned children and keep running. New listeners, sessions, inputs, and processing workers resolve the replaced binaries from the canonical install path. On Linux, Helmsman does not report success or stamp the new component version until the angel's `/proc/<pid>/exe` is the installed controller inode.
...
The reconciler may re-drive or restore a draining node only when the persisted operational-mode owner is `update-orchestrator`. An operator-owned drain or maintenance fence is never cleared by release reconciliation, even if an older update-state row still exists for that node.
...
- The edge container's `/opt/frameworks` and `/data/storage` volumes should be local filesystems for predictable atomic rename, capacity reporting, and media I/O behavior. On macOS prefer named volumes over virtiofs binds for I/O speed.
