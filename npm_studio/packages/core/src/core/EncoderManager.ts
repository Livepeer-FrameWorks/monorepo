...
import type { EncoderConfig, VideoEncoderSettings, AudioEncoderSettings, EncoderOverrides } from '../types';
...
export class EncoderManager extends TypedEventEmitter<EncoderManagerEvents> {
  ...
  private async createWorkerAsync(): Promise<Worker> {
    ...
    urlsToTry.push(
      ...
      { url: '/node_modules/@livepeer-frameworks/streamcrafter-core/dist/workers/encoder.worker.js', description: 'node_modules' },
...
export function createEncoderConfig(
  ...
  return {
    ...
    audio: {
      ...
      ...(overrides?.audio?.numberOfChannels !== undefined && { numberOfChannels: overrides.audio.numberOfChannels }),
    },
  };
}
