...
const VUMeter: React.FC<VUMeterProps> = ({ level, peakLevel }) => (
  <div className="fw-sc-vu-meter">
    <div
      className="fw-sc-vu-meter-fill"
      style={{ width: `${Math.min(level * 100, 100)}%` }}
    />
    <div
      className="fw-sc-vu-meter-peak"
...
const StreamCrafterInner: React.FC<StreamCrafterProps> = ({
  ...
  const {
    ...
    gatewayUrl && streamKey && !whipUrl
      ? { gatewayUrl, streamKey }
      : {}
  );
  ...
  const toggleSourceMute = useCallback((sourceId: string, currentMuted: boolean) => {
  ...
  const handleAudioProcessingChange = useCallback((newSettings: Partial<AudioProcessingSettings>) => {
    ...
    <ContextMenu>
          ...
          <div className={cn(
            ...
            <div className="fw-sc-content">
              ...
              <div className="fw-sc-preview-wrapper">
                ...
                <div className="fw-sc-preview">
                  ...
                  {/* Live Badge */}
                  {isStreaming && (
                    <div className="fw-sc-live-badge">Live</div>
                  )}
                    ...
                    <div className="fw-sc-section-body--flush">
                      <div className="fw-sc-sources">
                        {sources.map((source: MediaSource) => {
                          ...
                          const layer = compositor.activeScene?.layers.find(l => l.sourceId === source.id);
                            ...
                            <SourceRow
                              ...
                              onVisibilityToggle={() => {
                                if (compositor.activeSceneId && layer) {
                                  compositor.setLayerVisibility(compositor.activeSceneId, layer.id, !isVisibleInCompositor);
                                }
                              }}
                            />
                          );
            ...
            {!resolvedWhipUrl && !error && !endpointError && !isResolvingEndpoint && (
              <div className="fw-sc-error" style={{ borderLeftColor: 'hsl(40 80% 65%)' }}>
                <div className="fw-sc-error-title" style={{ color: 'hsl(40 80% 65%)' }}>Warning</div>
            ...
            {isResolvingEndpoint && (
              <div className="fw-sc-error" style={{ borderLeftColor: 'hsl(210 80% 65%)' }}>
                <div className="fw-sc-error-title" style={{ color: 'hsl(210 80% 65%)' }}>Resolving</div>
            ...
            <div className="fw-sc-actions">
              ...
              <div style={{ position: 'relative' }}>
                <button
                  ...
                  type="button"
                  className={cn('fw-sc-action-secondary', showSettings && 'fw-sc-action-secondary--active')}
                ...
                {showSettings && (
                  ...
                  >
                    {/* Quality Section */}
                    <div style={{ padding: '8px', borderBottom: '1px solid rgba(90, 96, 127, 0.3)' }}>
                      <div
                        style={{
                          fontSize: '10px',
                          color: '#565f89',
                        ...
                        {QUALITY_PROFILES.map(p => (
                          <button
                            ...
                            style={{
                              ...
                              background: qualityProfile === p.id ? 'rgba(122, 162, 247, 0.2)' : 'transparent',
                              color: qualityProfile === p.id ? '#7aa2f7' : '#a9b1d6',
                            }}
                          >
                            <div style={{ fontWeight: 500 }}>{p.label}</div>
                            <div style={{ fontSize: '10px', color: '#565f89' }}>{p.description}</div>
                          </button>
                        ))}
                      </div>
                    </div>
      ...
      <ContextMenuContent>
        ...
        {devMode && (
          <>
            <ContextMenuSeparator />
            <ContextMenuItem onClick={() => setIsAdvancedPanelOpen(!isAdvancedPanelOpen)} className="gap-2">
