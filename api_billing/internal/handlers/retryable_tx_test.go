...
func expectPrepaidTopupCreditBody(mock sqlmock.Sqlmock, balanceAfter int64) {
	mock.ExpectQuery(`(?s)SELECT status, tenant_id::text AS tenant_id.*FROM purser.pending_topups.*FOR UPDATE`).
		...
		WillReturnRows(pendingTopupRows("pending", checkoutTenantID, "mollie", 1500, "EUR", "sess-retry", ""))
	mock.ExpectExec("UPDATE purser.pending_topups").
		WithArgs("pay-retry", "sess-retry", "topup-retry", checkoutTenantID, "mollie", int64(1500), "EUR").
	...
	mock.ExpectExec("INSERT INTO purser.provider_settlements").
		WithArgs(checkoutTenantID, "mollie", "pay-retry", "topup-retry", nil, int64(1500), "EUR").
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec(`INSERT INTO purser.balance_transactions`).
		WithArgs(sqlmock.AnyArg(), checkoutTenantID, int64(1500), balanceAfter, "topup", "Card top-up via mollie", "topup-retry", "topup", "webhook", nil, "mollie checkout completed", "sess-retry", sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec("UPDATE purser.pending_topups").
	...
	mock.ExpectExec("INSERT INTO purser.billing_event_outbox").
		WithArgs(sameEventID{credited}, eventTopupCredited, checkoutTenantID, "", "topup", "topup-retry", sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(0, 1))
}
...
func TestPrepaidCheckoutCreditReplaysSerializationFailure(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := s.handlePrepaidCheckoutCompleted(context.Background(), "sess-retry", "pay-retry", checkoutTenantID, "topup-retry", 1500, "EUR", ProviderMollie, true); err != nil {
