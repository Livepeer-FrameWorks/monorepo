...
func (s *Service) applyProviderReversal(parentCtx context.Context, in providerReversalInput) (bool, error) {
	...
	err := database.WithRetryablePostgresTx(ctx, s.db, nil, func(tx *sql.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("read payment FX fields for reversal: %w", err)
		}
		...
		if paymentFX == nil {
			if !strings.EqualFold(paymentCurrency, billing.LedgerCurrency) {
				return fmt.Errorf("reversal %s references a %s payment without FX fields; retry after the EUR ledger conversion", in.providerReversalID, paymentCurrency)
			}
		...
		if err != nil {
			return fmt.Errorf("load earlier reversals of %s: %w", in.providerPaymentID, err)
		}
		reversalFX, err := paymentFX.Part(in.amountCents, prior.OriginalCents, prior.EurCents)
		if err != nil {
			return fmt.Errorf("apportion reversal %s: %w", in.providerReversalID, err)
		}
...
func (s *Service) updateInvoicePaymentStatus(provider, txID, invoiceID, newStatus string, emitEvent invoicePaymentEventWriter, evidence providerSettlementEvidence) (bool, error) {
	...
	err := database.WithRetryablePostgresTx(ctx, s.db, nil, func(tx *sql.Tx) error {
		...
		paymentID = payment.PaymentID
		foundInvoiceID := payment.InvoiceID
		...
		if err != nil {
			return fmt.Errorf("lock payment %s for settlement: %w", paymentID, err)
		}
		if newStatus == "confirmed" {
